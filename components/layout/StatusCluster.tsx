...
function Chip({ ok, label, Icon }: { ok: boolean; label: string; Icon: any }) {
  return (
    <div className="flex items-center gap-2 rounded-xl border border-zinc-800/70 bg-zinc-900/40 px-2 py-1 text-xs" role="status" aria-live="polite" aria-label={`${label} ${ok ? 'healthy' : 'down'}`}>
