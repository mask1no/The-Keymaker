...
export default function EngineHeader({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mb-4">
      ...
      <BadgePill tone={armedUntilLabel ? 'success' : 'muted'}>
        ARMED {armedUntilLabel ? `until ${armedUntilLabel}` : '(disarmed)'}
      </BadgePill>
      <BadgePill tone={dryRun ? 'warn' : 'danger'}>DryRun: {dryRun ? 'ON' : 'OFF'}</BadgePill>
      <BadgePill>Cluster: {cluster}</BadgePill>
      <button
        className="ml-auto text-sm text-zinc-300 hover:text-white underline"
