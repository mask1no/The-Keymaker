...
export default function ModeTiles({
  ...
  const Tile = ({
    ...
    <div
      onClick={onClick}
      className={`cursor-pointer rounded-xl border p-4 ${active ? 'border-violet-500 bg-violet-500/5' : 'border-k bg-[var(--k-surface)] hover:bg-zinc-900'}`}
    >
        ...
        <Tile
          ...
          onClick={async () => {
            ...
            await fetch('/api/ui/settings', {
              ...
              body: JSON.stringify({ mode: 'RPC_FANOUT' }),
            });
          }}
        />
      </div>
