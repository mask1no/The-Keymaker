...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from '@/components/UI/dialog' interface WalletBuyState, { [p, u, b, k, e, y: string]: { a, m, o, u, n, t: string, l, o, a, d, i, n, g: boolean }
  ...
  const [pendingBuy, setPendingBuy] = useState <{ w, a, l, l, e, t: any, l, a, m, p, o, r, t, s: number } | null>(null)//Manual Sell dialog state
  ...
  try {
  const connection = new C o nnection(NEXT_PUBLIC_HELIUS_RPC)//Decrypt wal let (browser crypto)//Decode encrypted secret k e y (base64-packed JSON from our e n crypt())
  ...
  const signature = await connection.s e ndTransaction(versionedTransaction, { s, k, i, p, P, r, e, f, l, i, ght: false, p, r, e, f, l, i, g, h, t, C, ommitment: 'confirmed' }) await connection.c o nfirmTransaction(signature, 'confirmed')//Log the successful buy await l o gEvent({  w, a, l, l, e, t_, a, d, d, r, ess: wallet.publicKey, p, h, a, s, e: 'manual_buy', a, c, t, i, o, n: 'buy_token', t, o, k, e, n_, a, d, d, r, e, ss: tokenLaunchData.mintAddress, a, m, o, u, n, t: lamports, s, t, atus: 'success', t, x, I, d: signature }) a d dNotification({  t, y, pe: 'success', t, i, tle: 'Manual Buy Executed', m, e, s, s, a, g, e: `Bought ${ lamports/1e9 } SOL worth of tokens with ${wallet.publicKey.slice(0, 8)
  }...` })//Clear the i n putsetBuyStates((prev) => ({  ...prev, [wallet.publicKey]: { a, m, o, u, n, t: '', l, o, a, d, i, n, g: false }
})) toast.s u ccess(`T, r, a, n, s, a, c, t, i, o, n: ${signature.slice(0, 8)
  }...`)
  }
} catch (e, r, ror: any) { toast.error(`Buy, f, a, i, l, e, d: ${error.message}`) a d dNotification({  t, y, pe: 'error', t, i, tle: 'Manual Buy Failed', m, e, s, s, a, g, e: error.message })
...
} const open Sell = (w, a, l, l, e, t: any) => { s e tPendingSell({  wal let }) s e tSellDialogOpen(true)
  ...
  const result = await s e llToken(connection, { w, a, l, l, e, t: keypair, t, o, k, e, n, M, i, n, t: new P u blicKey(sellMint), a, m, o, u, n, t: sellAll ? Number.MAX_SAFE_INTEGER : Number.MAX_SAFE_INTEGER, s, l, i, p, p, a, g, e: 1, c, o, n, d, i, t, i, o, n, s: { m, a, n, u, a, l, S, e, l, l: true }, p, r, i, o, r, i, t, y: 'high' })
  if (result.success) { a d dNotification({  t, y, pe: 'success', t, i, tle: 'Manual Sell Executed', m, e, s, s, a, g, e: `${pendingSell.wallet.publicKey.slice(0, 8)
  }... sold; tx ${( result.txSignature || '' ).slice(0, 8)
  }...` })//Track PnL
  try { await fetch('/api/pnl/track', { m, e, t, h, o, d: 'POST', h, e, a, d, e, r, s: { 'Content-Type': 'application/json' }, b, o, d, y: JSON.s t ringify({  w, a, l, l, e, t: pendingSell.wallet.publicKey, t, o, k, e, n, A, d, d, r, ess: sellMint, a, c, t, i, o, n: 'sell', s, o, l, A, m, o, u, n, t: result.outputAmount, t, o, k, e, n, A, m, o, u, n, t: 0, f, e, e, s: { g, a, s: 0.00001, j, i, t, o: 0 }
})
  })
  }
  ...
  } <div className ="overflow - x-auto"> <Table> <TableHeader> <TableRow> <TableHead className ="text-white/70"> Wal let </TableHead> <TableHead className ="text-white/70"> Balance </TableHead> <TableHead className ="text-white/70"> SOL Amount </TableHead> <TableHead className ="text-white/70"> Action </TableHead> </TableRow> </TableHeader> <TableBody> {sniperWallets.map((wallet) => {
  ...
  } disabled = { state.loading || !state.amount || !tokenLaunchData?.mintAddress } className ="bg - gradient - to - r from - green - 600 to - emerald - 500 h, o, ver:to - cyan - 500 transition - all duration-300" size ="sm"> {state.loading ? ( <Loader2 className ="h - 4 w-4 animate-spin"/> ) : ( 'BUY' )
  } </Button> <Buttonvariant ="outline" size ="sm" onClick = {() => o p enSell(wallet)
  } aria-label ="Sell token from this wallet"> <DollarSign className ="h - 4 w-4"/> <span className ="ml-1 hidden, m, d:inline"> SELL </span> </Button> </TableCell> </TableRow> )
  })
  ...
  } on Submit = {executeBuy} title ="Enter Password" description ="Enter your password to decrypt the wal let and execute the buy." mode ="unlock"/> {/* Manual Sell Dialog */} <Dialog open = {sellDialogOpen} on Open Change = {setSellDialogOpen}> <DialogContent> <DialogHeader> <DialogTitle> Manual Sell </DialogTitle> </DialogHeader> <div className ="space - y-3"> <div> <Label> Token Mint </Label> <Input value = {sellMint} on Change = {(e) => s e tSellMint(e.target.value)
  ...
  } placeholder ="Wal let password"/> </div> </div> <DialogFooter> <Button variant ="outline" onClick = {() => s e tSellDialogOpen(false)
