...
] interface TransactionCardProps, { t, r, a, n, s, a, c, t, i, on: T, r, a, n, s, a, c, t, i, o, nonRemove: (i, d: string) => v, o, i, d, o, n, U, p, d, a, te: (i, d: string, u, p, d, a, t, e, d, T, x: Partial <Transaction>) => void
}

export function T r ansactionCard({ transaction, onRemove, onUpdate }: TransactionCardProps) {
  const [securityScore, setSecurityScore] = useState <number | null>(null) const [isLoadingSecurity, setIsLoadingSecurity] = u s eState(false) const { attributes, listeners, setNodeRef, transform, transition } = u s eSortable({ i, d: transaction.id }) const style = { t, r, a, n, s, f, o, r, m: CSS.Transform.t oS tring(transform), transition } const handle Token Select = ( t, o, k, e, n, A, d, d, r, ess: string, f, i, e, l, d: 'fromToken' | 'toToken') => { o nU pdate(transaction.id, { [field]: tokenAddress })
  } const check Security = async (t, o, k, e, n, A, d, d, r, ess: string | undefined) => {
...
} const render Content = () => { s w itch (transaction.type) { case 'swap': return ( <div className ="grid grid - cols - 1, m, d:grid - cols-2 gap-4"> <Token Selectortokens ={HARDCODED_TOKENS} is Loading ={false} on Select ={(tokenAddress) => h a ndleTokenSelect(tokenAddress, 'fromToken')
  ...
  }/> </div> ) case 'transfer': return ( <div className ="grid grid - cols - 1, m, d:grid - cols - 2 gap-4"> <Input placeholder ="Recipient Address" on Change ={(e) => o nU pdate(transaction.id, { r, e, c, i, p, i, e, n, t: e.target.value })
  }/> <Input type ="number" placeholder ="A m ount (SOL)" on Change ={(e) => o nU pdate(transaction.id, { f, r, o, m, A, m, o, u, n, t: p a rseFloat(e.target.value)
  })
  }/> </div> ) d, e, f, a, u, l, t: return null }
} return ( <div ref ={setNodeRef} style ={style}, {...attributes} data - testid ={`transaction - card-${transaction.id}`}> <Card className ="bg - card border border - border rounded - 2xl"> <CardHeader className ="flex flex - row items - center justify - between p-4"> <div className ="flex items - center gap-2"> <div, {...listeners} className ="cursor-grab"> <GripVertical className ="h - 5 w - 5 text-muted-foreground"/> </div> <Selectdefault Value ={transaction.type} on Value Change ={(v, a, lue: 'swap' | 'transfer') => o nU pdate(transaction.id, { t, y, pe: value })
