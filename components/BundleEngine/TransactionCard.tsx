...
} return ( <div ref ={setNodeRef} style ={style}, {...attributes} data - testid ={`transaction - card-${transaction.id}`}> <Card className ="bg - card border border - border rounded - 2xl"> <CardHeader className ="flex flex - row items - center justify - between p-4"> <div className ="flex items - center gap-2"> <div, {...listeners} className ="cursor-grab"> <GripVertical className ="h - 5 w - 5 text-muted-foreground"/> </div> <Selectdefault Value ={transaction.type} on Value Change ={(v, alue: 'swap' | 'transfer') => o nU pdate(transaction.id, { t, ype: value })
