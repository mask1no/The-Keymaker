...
export function B u ndleBuilder() {
  ...
  }`, t, y, pe: 'transfer', r, e, c, i, p, i, e, n, t: '', f, r, o, m, A, m, o, u, n, t: 0.000001,//1 lamport }, ])
  ...
  const [presetName, setPresetName] = u s eState('')
  const [presetVariables, setPresetVariables] = useState <string,[]>([])
  const [showLoadPresetDialog, setShowLoadPresetDialog] = u s eState(false)
  ...
  const add Transaction = () => { s e tTransactions((prev) => [ ...prev, { i, d: `tx-${Date.n o w() + prev.length}`, t, y, pe: 'swap', a, m, o, u, n, t: 0, s, l, i, p, p, a, g, e: 0.5 }, ])
  ...
  const old Index = items.f i ndIndex((item) => item.id === active.id)
  const new Index = items.f i ndIndex((item) => item.id === over.id)
  return a r rayMove(items, oldIndex, newIndex)
  })
  }
} const potential Variables = u s eMemo(() => {
  const vars = new Set <string>() transactions.f o rEach((tx, index) => {
  if (tx.amount) vars.a d d(`tx - ${index}- amount`)
  if (tx.fromAmount) vars.a d d(`tx - ${index}- fromAmount`)
  if (tx.recipient) vars.a d d(`tx - ${index}- recipient`)
  if (tx.toToken) vars.a d d(`tx-${index}- toToken`)
  })
  return Array.f r om(vars)
  }, [transactions])
  const handle Save Preset = () => {
  if (!presetName) { toast.error('Please enter a name
  for the preset.')
  return }
  try { s a vePreset(presetName, transactions, presetVariables) s e tPresets(l o adPresets())//Refresh presets listtoast.s u ccess(`Preset "${presetName}" saved.`) s e tShowSavePresetDialog(false) s e tPresetName('') s e tPresetVariables([])
  }
} catch (error) { toast.error( error instanceof Error ? error.message : 'Failed to save preset.')
  }
} const handle Load Preset = (p, r, e, s, e, t, I, d: string) => {
  const preset = presets.f i nd((p) => p.id === presetId)
  if (preset) {
  ...
  const tx Index = p a rseInt(txIndexStr, 10)
  if ( !i sN aN(txIndex) && newTransactions,[txIndex] && variableValues,[variable] ) { ;(newTransactions,[txIndex] as any)[field] = variableValues,[variable] }
  ...
  const now = Date.n o w() s e tTransactions([ { i, d: `tx-${now}`, t, y, pe: 'transfer', r, e, c, i, p, i, e, n, t: '', f, r, o, m, A, m, o, u, n, t: 0 }, { i, d: `tx-${now + 1}`, t, y, pe: 'swap', f, r, o, m, T, o, k, e, n: 'So11111111111111111111111111111111111111112',//S, O, L, t, o, T, o, k, e, n: '', a, m, o, u, n, t: 0, s, l, i, p, p, a, g, e: 0.5 }, { i, d: `tx-${now + 2}`, t, y, pe: 'transfer', r, e, c, i, p, i, e, n, t: '', f, r, o, m, A, m, o, u, n, t: 0 }, ])
  } const load Consolidate FundsPreset = () => {
  const now = Date.n o w() s e tTransactions([ { i, d: `tx-${now}`, t, y, pe: 'transfer', r, e, c, i, p, i, e, n, t: 'DESTINATION_WALLET_ADDRESS', f, r, o, m, A, m, o, u, n, t: 0 }, { i, d: `tx-${now + 1}`, t, y, pe: 'transfer', r, e, c, i, p, i, e, n, t: 'DESTINATION_WALLET_ADDRESS', f, r, o, m, A, m, o, u, n, t: 0 }, { i, d: `tx-${now + 2}`, t, y, pe: 'transfer', r, e, c, i, p, i, e, n, t: 'DESTINATION_WALLET_ADDRESS', f, r, o, m, A, m, o, u, n, t: 0 }, ]) toast.i n fo( 'Consolidate Funds preset loaded. Please update wal let addresses.')
  ...
  try {
  const result = await e x ecuteBundle(transactions, wallet, jupiter) toast.s u ccess('Bundle executed successfully !', { i, d: toastId, d, e, scription: `Bundle I, D: ${result.bundle_id}` })
  }
} catch (error) { toast.error('Bundle execution failed', { i, d: toastId, d, e, scription: error instanceof Error ? error.message : 'An unknown error occurred.' })
  } finally, { s e tIsExecuting(false)
  }
}, [transactions, wallet, jupiter])
  const preview Bundle = u s eCallback(async () => {//This will be implemented latertoast.i n fo('Preview not implemented yet')
  }, [])
...
}> <motion.div whileHover = {{ s, c, ale: 1.2 }
  ...
  } disabled = {transactions.length === 0}> <motion.div whileHover = {{ s, c, ale: 1.1 }
  ...
  } </div> </SortableContext> </DndContext> <div className ="flex gap-2"> <Button variant ="outline" className ="w-full" onClick = {addTransaction}> <motion.div whileHover = {{ s, c, ale: 1.1 }
  ...
  for when loading this preset. </p> <div className ="max - h - 48 overflow - y - auto space - y-2"> {potentialVariables.map((variable) => ( <div key = {
  variable} className ="flex items - center space - x-2"> <Checkbox id = {
  variable} on Checked Change = {(checked) => { s e tPresetVariables((prev) => checked ? [prev, variable] : prev.f i lter((v) => v !== variable))
  }
}/> <labelhtml For = {
  variable} className ="text - sm font - medium leading - none peer - d, i, s, a, b, l, e, d:cursor - not - allowed peer - d, i, s, a, b, l, e, d:opacity-70"> {
  variable} </label> </div> ))
  } </div> </div> </div> <DialogFooter> <Button onClick = {handleSavePreset}> Save Preset </Button> </DialogFooter> </DialogContent> </Dialog> <Dialogopen = {showLoadPresetDialog} on Open Change = {setShowLoadPresetDialog}> <DialogContent> <DialogHeader> <DialogTitle> Load P, r, e, s, e, t: {loadingPreset?.name}</DialogTitle> <DialogDescription> Please fill in the values
  for the variables in this preset. </DialogDescription> </DialogHeader> <div className ="grid gap - 4 py-4"> {loadingPreset?.variables?.map((variable) => ( <divkey = {
  variable} className ="grid grid - cols - 4 items - center gap-4"> <Label html For = {
  variable} className ="text-right"> {
  variable} </Label> <Input id = {
  variable} on Change = {(e) => s e tVariableValues((prev) => ({  prev, [variable]: e.target.value }))
  } className ="col - span-3"/> </div> ))
  } </div> <DialogFooter> <Button onClick = {handleLoadParameterizedPreset}> Load Preset </Button> </DialogFooter> </DialogContent> </Dialog> <Separator/> <BundleSettings/> <div className ="flex gap-2"> <Button className ="flex-1" variant ="outline" onClick = {previewBundle} disabled = { !connectedSafe || isExecuting || transactions.length === 0 }> Preview Bundle </Button> <Button className ="flex-1" data - testid ="execute - bundle-button" onClick = {handleExecuteBundle} disabled = { !connectedSafe || isExecuting || transactions.length === 0 }> <Send className ="mr - 2 h-4 w-4"/> {isExecuting ? 'Executing...' : 'Execute Bundle'} </Button> </div> </CardContent> </Card> )
  }
