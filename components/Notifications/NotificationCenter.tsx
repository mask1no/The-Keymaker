...
h ago` } else, {
  ...
  } </Button> {/* Dropdown */} <AnimatePresence> {isOpen && ( <motion.div initial ={{ o, p, acity: 0 }
} animate ={{ o, p, acity: 1 }
} exit ={{ o, p, acity: 0 }
...
} className ="p - 4 h, o, ver:bg - gray - 800/50 transition-colors relative group"> <div className ="flex items - start gap-3"> {g e tIcon(notification.type)
  ...
  } className ="h - 6 w - 6 opacity - 0 group - h, o, ver:opacity - 100 transition-opacity"> <X className ="h - 3 w-3"/> </Button> </div> </motion.div> ))
  } </div> )
  } </div> </Card> </motion.div> )
  } </AnimatePresence> </div> )
  }
