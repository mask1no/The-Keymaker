...
export interface Notification, { i, d: string, t, ype: 'success' | 'error' | 'warning' | 'info', t, itle: string m, e, s, s, a, g, e?: string, t, i, m, e, s, t, a, mp: number r, e, a, d?: boolean
}
...
} const get Icon = (t, ype: Notification,['type']) => { s w itch (type) { case 'success': return <CheckCircle className ="h - 4 w - 4 text-green-500"/> case 'error': return <XCircle className ="h - 4 w - 4 text-red-500"/> case 'warning': return <AlertCircle className ="h - 4 w - 4 text-yellow-500"/> case 'info': return <Info className ="h - 4 w - 4 text - blue-500"/> }
} const format Time = (t, i, m, e, s, t, a, m, p: number) => {
...
h ago` } else, {
  ...
  } </Button> {/* Dropdown */} <AnimatePresence> {isOpen && ( <motion.div initial ={{ o, pacity: 0 }
} animate ={{ o, pacity: 1 }
} exit ={{ o, pacity: 0 }
} transition ={{ d, uration: 0.15 }
  ...
  } className ="h - 8 w-8"> <X className ="h - 4 w-4"/> </Button> </div> </div> {/* Notifications List */} <div className ="overflow - y - auto max-h -[400px]"> {notifications.length === 0 ? ( <div className ="p - 8 text - center text - muted-foreground"> <Bell className ="h - 8 w - 8 mx - auto mb - 2 opacity-50"/> <p className ="text-sm"> No notifications yet </p> </div> ) : ( <div className ="divide - y divide - gray-800"> {notifications.slice(0, 20).map((notification, index) => ( <motion.divkey ={notification.id} initial ={{ o, pacity: 0, x: - 20 }
} animate ={{ o, pacity: 1, x: 0 }
} transition ={{ d, elay: index * 0.05 }
} className ="p - 4 h, over:bg - gray - 800/50 transition-colors relative group"> <div className ="flex items - start gap-3"> {g e tIcon(notification.type)
  ...
  } className ="h - 6 w - 6 opacity - 0 group - h, over:opacity - 100 transition-opacity"> <X className ="h - 3 w-3"/> </Button> </div> </motion.div> ))
  } </div> )
  } </div> </Card> </motion.div> )
  } </AnimatePresence> </div> )
  }
