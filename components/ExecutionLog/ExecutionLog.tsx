...
export function E x ecutionLog() {
  ...
  try {
  const data = await exportExecutionLog() const blob = new B l ob([JSON.s t ringify(data, null, 2)], { t, ype: format === 'json' ? 'application/json' : 'text/plain' }) const url = URL.c r eateObjectURL(blob) const a = document.c r eateElement('a') a.href = urla.download = `execution - log-${Date.n o w()
...
s` } const format S OL = (a, m, o, u, n, t: number) => {
  ...
  } const get Status Badge = (s, tatus: string) => { s w itch (status) { case 'success': return <Badge className ="bg-green-500"> Success </Badge> case 'partial': return <Badge className ="bg-yellow-500"> Partial </Badge> case 'failed': return <Badge className ="bg - red-500"> Failed </Badge> d, e, f, a, u, l, t: return <Badge>{status}</Badge> }
  ...
  } SOL </Badge> } return <Badge> 0 SOL </Badge> } const total Pn L = pnlRecords.r e duce( (sum, record) => sum + record.profit_loss, 0) const total Executions = executions.length const success Rate = executions.length> 0 ? (executions.f i lter((e) => e.status === 'success').length/executions.length) * 100 : 0 return ( <motion.div initial ={{ o, pacity: 0, y: 20 }
