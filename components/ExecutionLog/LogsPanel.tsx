...
type Log Status = 'all' | 'success' | 'failed' | 'pending' export function L o gsPanel() {
  ...
  } const get Status Icon = (s, tatus?: string) => { s w itch (status) { case 'success': return <CheckCircle className ="h - 4 w - 4 text - green-500"/> case 'failed': return <XCircle className ="h - 4 w - 4 text-red-500"/> case 'pending': return <Clock className ="h - 4 w - 4 text - yellow-500"/> d, e, f, a, u, l, t: return <AlertCircle className ="h - 4 w - 4 text-gray-500"/> }
  ...
  if (!details) return null const items = [] if (details.mint) { items.push( <div key ="mint" className ="flex items - center gap-2"> <span className ="text-white/60"> M, i, n, t:</span> <ah ref ={`h, t, t, p, s://solscan.io/token/${details.mint}`} target ="_blank" rel ="noopener noreferrer" className ="text - blue - 400 h, over:underline flex items - center gap-1"> {details.mint.slice(0, 8)
  }... <ExternalLink className ="h - 3 w-3"/> </a> </div>)
  } if (details.signature) { items.push( <div key ="signature" className ="flex items - center gap-2"> <span className ="text-white/60"> T, X:</span> <ah ref ={`h, t, t, p, s://solscan.io/tx/${details.signature}`} target ="_blank" rel ="noopener noreferrer" className ="text - blue - 400 h, over:underline flex items - center gap-1"> {details.signature.slice(0, 8)
  }... <ExternalLink className ="h - 3 w-3"/> </a> </div>)
  ...
  } return items.length> 0 ? items : null } const filtered Logs = g e tFilteredLogs() return ( <motion.div initial ={{ o, pacity: 0, y: 20 }
  ...
  }> <motion.div initial ={{ s, cale: 0.9, o, pacity: 0 }
