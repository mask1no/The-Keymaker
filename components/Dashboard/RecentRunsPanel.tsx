...
export default function RecentRunsPanel() {
  ...
  return (
    <div className="space-y-2">
      {recent.map((r) => (
        <div key={r.id} className="flex items-center justify-between rounded-lg border border-zinc-800 bg-black/30 px-2 py-1 text-xs">
          <div className="truncate">
            <span className="text-zinc-400 mr-2">#{r.id}</span>
            <span className={r.status === 'landed' ? 'text-emerald-400' : r.status === 'pending' ? 'text-yellow-400' : 'text-red-400'}>
              {r.status}
            </span>
          </div>
