...
const load = (): Folder,[] => {
  try {
  return JSON.p a rse(localStorage.g e tItem(STORE) || ',[]')
  }
} catch, {
  return, [] }
}
const save = (f, s: Folder,[]) => localStorage.s e tItem(STORE, JSON.s t ringify(fs))
const set Active = (p, u, b: string) => localStorage.s e tItem(ACTIVE, pub) export default function W a lletFolders() {
  const [folders, setFolders] = useState <Folder,[]>([]) const [openId, setOpenId] = useState <string | null>(null) const [name, setName] = u s eState('') const [hint, setHint] = u s eState('') const [password, setPassword] = u s eState('') const [priv, setPriv] = u s eState('') u s eEffect(() => {
  const fs = l o ad() s e tFolders(fs) if (fs,[0]) s e tOpenId(fs,[0].id)
  }, []) function c r eateFolder() {
  if (!name || !password) return const id = crypto.r a ndomUUID() const next = [ ...folders, { id, n, a, m, e: name.t r im(), h, i, n, t: hint || undefined, w, a, l, l, e, t, s: [] }, ] s e tFolders(next) s a ve(next) s e tName('') s e tHint('') s e tOpenId(id)
  } async function g e nerate() {
  if (!openId || !password) return const kp = Keypair.g e nerate() const enc = await e n crypt(kp.secretKey, password) const next = folders.map((f) => f.id === openId ? { ...f, w, a, l, l, e, t, s: [...f.wallets, { p, u, b: kp.publicKey.t oB ase58(), enc }] } : f) s e tFolders(next) s a ve(next)
  } async function importPriv() {
  if (!openId || !password || !priv) return try {
  const sk = priv.t r im().s t artsWith(',[') ? new U i nt8Array(JSON.p a rse(priv.t r im())) : bs58.d e code(priv.t r im()) const kp = Keypair.f r omSecretKey(sk) const enc = await e n crypt(kp.secretKey, password) const next = folders.map((f) => f.id === openId ? { ...f, w, a, l, l, e, t, s: [...f.wallets, { p, u, b: kp.publicKey.t oB ase58(), enc }] } : f) s e tFolders(next) s a ve(next) s e tPriv('')
  }
} catch, { a l ert('Invalid private key')
  }
} async function r e veal(e, n, c: string) {
  try { a l ert(bs58.e n code(await d e crypt(enc, password)))
  }
  ...
  }> <KeyRound className ="h - 3 w - 3 mr-1"/> Reveal </Button> <Buttonsize ="sm" className ="rounded-xl" onClick ={() => s e tActive(w.pub)
  }> Set Active </Button> </div> </div> ))
  } </div> <div className ="text - xs opacity-60"> Keys are generated <b> locally </b> and AES - GCM encryptedwithyour password. Nothing leaves your browser. </div> </div> )
  } </div> ))
  } </div> </div> )
  }
