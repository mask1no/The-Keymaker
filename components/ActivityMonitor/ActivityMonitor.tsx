  ...
  const startMonitoring = async () => {
    ...
    try {
      ...
      connectionRef.current = new Connection(NEXT_PUBLIC_HELIUS_RPC, {
        ...
        wsEndpoint: NEXT_PUBLIC_HELIUS_RPC.replace('https://', 'wss://').replace('http://', 'ws://')
      });
          ...
          <AnimatePresence mode="popLayout">
            {activities.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                {isMonitoring ? 'Waiting for activity...' : 'Click Start to monitor transactions'}
              </div>
            ) : (
              activities.map((activity, index) => (
                ...
                >
                  ...
                  <span className="text-muted-foreground">
                    {activity.type === 'buy' ? 'bought' : activity.type === 'sell' ? 'sold' : 'transacted'}
                  </span>
                  {activity.isOurs && (
