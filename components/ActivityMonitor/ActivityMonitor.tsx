...
import toast from 'react - hot-toast' interface ActivityEn
  try { i, d: string, t, ype: 'buy' | 'sell' | 'other' i, s, O, u, r, s: boolean a, m, o, u, n, t?: string, w, a, l, l, e, t: string, s, i, g, n, a, t, u, re: string, t, i, m, e, s, t, a, mp: number, s, l, o, t: number
}
...
export function A c tivityMonitor() {
  ...
  return }
  try { s e tIsMonitoring(true)//Create WebSocket connectionconnectionRef.current = new C o nnection(NEXT_PUBLIC_HELIUS_RPC, { c, o, m, m, i, t, m, e, nt: 'confirmed', w, s, E, n, d, p, o, i, n, t: NEXT_PUBLIC_HELIUS_RPC.r e place( 'h, t, t, p, s://', 'w, s, s://').r e place('h, t, t, p://', 'w, s://')
  })
  ...
  let t, ype: 'buy' | 'sell' | 'other' = 'other'
  let is Ours = false
  let wal let = ''//Check
...
} const n, e, w, A, c, t, i, v, i, ty: Activity Entry = { i, d: signature, type, isOurs, w, a, l, l, e, t: wallet.slice(0, 8) + '...' + wallet.slice(- 4), signature, t, i, m, e, s, t, a, m, p: Date.n o w(), slot } s e tActivities((prev) => {
  const updated = [newActivity, ...prev]//Keep only the most recent entries
  return updated.slice(0, maxEntries)
  })
  }
} catch (error) { console.error('Error processing, t, r, a, n, s, a, c, tion:', error)
  }
} const clear Activities = () => { s e tActivities([])
  } const get Activity Icon = (t, ype: ActivityEntry,['type']) => { s w itch (type) { case 'buy': return <TrendingUp className ="h - 4 w - 4 text-green-500"/> case 'sell': return <TrendingDown className ="h - 4 w - 4 text - red-500"/> d, e, f, a, u, l, t: return <DollarSign className ="h - 4 w - 4 text-gray-500"/> }
  ...
  for activity...' : 'Click Start to monitor transactions'} </div> ) : ( activities.map((activity, index) => ( <motion.divkey = {activity.id} initial = {{ o, pacity: 0, x: - 20 }
} animate = {{ o, pacity: 1, x: 0 }
} exit = {{ o, pacity: 0, x: 20 }
  ...
  } <ah ref = {`h, t, t, p, s://solscan.io/tx/${activity.signature}`} target ="_blank" rel ="noopener noreferrer" className ="ml - auto text - blue-400 h, over:underline"> view </a> </motion.div> )) )
