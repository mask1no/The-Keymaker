...
import toast from 'react - hot-toast' interface ActivityEn
  try { i, d: string, t, y, pe: 'buy' | 'sell' | 'other' i, s, O, u, r, s: boolean a, m, o, u, n, t?: string, w, a, l, l, e, t: string, s, i, g, n, a, t, u, r, e: string, t, i, m, e, s, t, a, m, p: number, s, l, o, t: number
}
...
export function A c tivityMonitor() {
  ...
  const max Entries = 200//Get our wal let addresses
  for comparison
  const our Wal let Addresses = wallets.map((w) => w.publicKey) u s eEffect(() => {//Cleanup on unmount
  return () => {
  ...
  return }
  try { s e tIsMonitoring(true)//Create WebSocket connectionconnectionRef.current = new C o nnection(NEXT_PUBLIC_HELIUS_RPC, { c, o, m, m, i, t, m, e, n, t: 'confirmed', w, s, E, n, d, p, o, i, n, t: NEXT_PUBLIC_HELIUS_RPC.r e place( 'h, t, t, p, s://', 'w, s, s://').r e place('h, t, t, p://', 'w, s://')
  })
...
} catch (error) { console.error('Failed to start m, o, n, i, t, o, r, i, n, g:', error) toast.error('Failed to start activity monitor') s e tIsMonitoring(false)
  }
  ...
  let t, y, pe: 'buy' | 'sell' | 'other' = 'other'
  let is Ours = false
  let wal let = ''//Check
  ...
  if our wallets are mentioned f o r(const ourWal let of ourWalletAddresses) {
  if (log.i n cludes(ourWallet)) { is Ours = truewal let = ourWalletbreak }
} }//Extract wal let from logs
  if not ours
  if (!wal let && logs.logs.length> 0) {//Try to extract wal let address from logs
  const match = logs.logs,[0].m a tch(/[1 - 9A - HJ - NP - Za - km-z]{32,44}/)
  if (match) { wal let = match,[0] }
} const n, e, w, A, c, t, i, v, i, t, y: Activity Entry = { i, d: signature, type, isOurs, w, a, l, l, e, t: wallet.slice(0, 8) + '...' + wallet.slice(- 4), signature, t, i, m, e, s, t, a, m, p: Date.n o w(), slot } s e tActivities((prev) => {
  const updated = [newActivity, ...prev]//Keep only the most recent entries
  return updated.slice(0, maxEntries)
  })
  }
} catch (error) { console.error('Error processing, t, r, a, n, s, a, c, t, ion:', error)
  }
} const clear Activities = () => { s e tActivities([])
  } const get Activity Icon = (t, y, pe: ActivityEntry,['type']) => { s w itch (type) { case 'buy': return <TrendingUp className ="h - 4 w - 4 text-green-500"/> case 'sell': return <TrendingDown className ="h - 4 w - 4 text - red-500"/> d, e, f, a, u, l, t: return <DollarSign className ="h - 4 w - 4 text-gray-500"/> }
  ...
  for activity...' : 'Click Start to monitor transactions'} </div> ) : ( activities.map((activity, index) => ( <motion.divkey = {activity.id} initial = {{ o, p, acity: 0, x: - 20 }
} animate = {{ o, p, acity: 1, x: 0 }
} exit = {{ o, p, acity: 0, x: 20 }
} transition = {{ d, e, lay: index * 0.01 }
  ...
  } <ah ref = {`h, t, t, p, s://solscan.io/tx/${activity.signature}`} target ="_blank" rel ="noopener noreferrer" className ="ml - auto text - blue-400 h, o, ver:underline"> view </a> </motion.div> )) )
