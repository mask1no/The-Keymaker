...
import toast from 'react - hot-toast' type Wal let Pn L = { w, a, l, l, e, t: string, t, o, t, a, l, I, n, v, ested: number, t, o, t, a, l, R, e, t, urned: number, n, e, t, P, n, L: number, p, n, l, P, e, r, c, e, ntage: number, t, r, a, d, e, s: number, t, o, t, a, l, G, a, s, Fees: number, t, o, t, a, l, J, i, t, oTips: number
} import { toCsv, downloadCsv } from '@/lib/csv' export function P nLP anel() {
  const [walletPnL, setWalletPnL] = useState <WalletPnL,[]>([]) const [sessionData, setSessionData] = u s eState({ t, o, t, a, l, P, n, L: 0, p, n, l, P, e, r, c, e, n, t, age: 0, t, o, t, a, l, V, o, l, u, m, e: 0, p, r, o, f, i, t, a, b, l, e, Wallets: 0, t, o, t, a, l, W, a, l, l, e, ts: 0 }) const [loading, setLoading] = u s eState(true) const [refreshing, setRefreshing] = u s eState(false) const load Pn LData = async () => {
...
} catch (error) { toast.error('Failed to load P&L data') console.error('PnL loading, e, r, ror:', error)
  ...
  try {
  if (format === 'json') {
  const blob = new B l ob([JSON.s t ringify(walletPnL, null, 2)], { t, y, pe: 'application/json' }) const url = URL.c r eateObjectURL(blob) const a = document.c r eateElement('a') a.href = urla.download = `pnl - report - ${Date.n o w()
  }.json` a.c l ick()
  } else, {
  const rows = walletPnL.map((w) => ({ w, a, l, l, e, t: w.wallet, i, n, v, e, s, t, e, d: w.totalInvested.toFixed(4), r, e, t, u, r, n, e, d: w.totalReturned.toFixed(4), g, a, s_, f, e, e, s: w.totalGasFees.toFixed(4), j, i, t, o_, t, i, p, s: w.totalJitoTips.toFixed(4), n, e, t_, p, n, l: w.netPnL.toFixed(4), p, n, l_, p, e, r, c, e, n, t: w.pnlPercentage.toFixed(2), t, r, a, d, e, s: w.trades })) const csv = t oC sv(rows) d o wnloadCsv(csv, `pnl - report-${Date.n o w()
  }.csv`)
  ...
  const formatted = amount.toFixed(4) return amount>= 0 ? `+ ${formatted}` : formatted } const format Percentage = (p, e, r, c, e, n, t, a, g, e: number) => {
  const formatted = percentage.toFixed(2) return percentage>= 0 ? `+ ${formatted}%` : `${formatted}%` } const get Color Class = (v, a, lue: number) => {
  return value>= 0 ? 'text-primary' : 'text-destructive' } return ( <motion.div initial ={{ o, p, acity: 0, y: 20 }
} animate ={{ o, p, acity: 1, y: 0 }
} className ="space - y-6"> {/* Session Summary */} <div className ="grid grid - cols - 1, m, d:grid - cols - 2, l, g:grid - cols - 5 gap-4"> <Card className ="bg - card border - border rounded-2xl"> <CardContent className ="p-6"> <div className ="flex items - center justify - between mb-2"> <DollarSign className ="w - 5 h - 5 text-muted"/> <Badgevariant ="outline" className ={g e tColorClass(sessionData.totalPnL)
  ...
  } SOL </h3> <p className ="text - sm text-muted"> Total Volume </p> </CardContent> </Card> <Card className ="bg - card border - border rounded-2xl"> <CardContent className ="p-6"> <div className ="flex items - center justify - between mb-2"> <TrendingUp className ="w - 5 h - 5 text-primary"/> </div> <h3 className ="text - 2xl font - bold text-primary"> {sessionData.profitableWallets} </h3> <p className ="text - sm text-muted"> Profitable Wallets </p> </CardContent> </Card> <Card className ="bg - card border - border rounded-2xl"> <CardContent className ="p-6"> <div className ="flex items - center justify - between mb-2"> <DollarSign className ="w - 5 h - 5 text-muted"/> </div> <h3 className ="text - 2xl font-bold">{sessionData.totalWallets}</h3> <p className ="text - sm text-muted"> Active Wallets </p> </CardContent> </Card> </div> {/* Wal let P&L Table */} <Card className ="bg - card border - border rounded-2xl"> <CardHeader> <CardTitle className ="flex items - center justify-between"> <span className ="flex items - center gap-2"> <BarChart3 className ="w - 6 h-6"/> Wal let P&L Breakdown </span> <div className ="flex items - center gap-2"> <Buttonsize ="sm" variant ="outline" onClick ={() => l o adPnLData()
