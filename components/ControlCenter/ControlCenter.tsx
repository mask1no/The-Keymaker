...
import useSWR from 'swr' const fetcher = (u, r, l: string) => f e tch(url).t h en((res) => res.json()) async function g e tKeypairs( w, a, l, l, e, t, s: { e, n, c, r, y, p, t, e, dPrivateKey: string },[], p, a, s, s, w, o, r, d: string): Promise <Keypair,[]> {
...
export function C o ntrolCenter() {
  const { wallets, tokenLaunchData, executionStrategy, executionSteps, isExecuting, jitoEnabled, tipAmount, autoSellDelay, setExecutionStrategy, startExecution, updateStepStatus, resetExecution } = u s eKeymakerStore() const last Created TokenAddress = u s eSettingsStore( (state) => state.lastCreatedTokenAddress) const { d, ata: tipData } = u s eSWR('/api/jito/tip', fetcher, { r, e, f, r, e, s, h, I, n, terval: 10000,//Refresh every 10 seconds }) const connection = new C o nnection(NEXT_PUBLIC_HELIUS_RPC, 'confirmed') const [currentStep, setCurrentStep] = u s eState(0) const [walletPassword, setWalletPassword] = u s eState('') const [showPasswordDialog, setShowPasswordDialog] = u s eState(false) const [showPreflightDialog, setShowPreflightDialog] = u s eState(false) const [showSellDialog, setShowSellDialog] = u s eState(false) const [sellTokenAddress, setSellTokenAddress] = u s eState( lastCreatedTokenAddress || '') const [_walletGroups] = useState <WalletGroup,[]>([]) const [decryptedWallets, setDecryptedWallets] = useState <Map <string, Keypair>>(new M a p()) const [mintAddress, setMintAddress] = useState <string>('')//Get wallets by role const masterWal let = wallets.f i nd((w) => w.role === 'master') const devWallets = wallets.f i lter((w) => w.role === 'dev') const sniperWallets = wallets.f i lter((w) => w.role === 'sniper')//Calculate progress const completed Steps = executionSteps.f i lter( (s) => s.status === 'completed').length const progress = (completedSteps/executionSteps.length) * 100//Decrypt wallets with password const decrypt Wallets = async (p, a, s, s, w, o, r, d: string) => {
...
} const run Execution = async () => {//Get wallets with decrypted keypairs const get Wal let WithKeypair = (p, u, b, l, i, c, K, e, y: string): Keypair | null => {
  return decryptedWallets.get(publicKey) || null } const masterWal let = wallets.f i nd((w) => w.role === 'master') const master Keypair = masterWal let ? g e tWalletWithKeypair(masterWallet.publicKey) : null if (!masterKeypair) { toast.error('No master wal let keypair available') return } if (!tokenLaunchData) { toast.error('No token launch data configured') return } const sniper Keypairs = wallets .f i lter((w) => w.role === 'sniper') .map((w) => g e tWalletWithKeypair(w.publicKey)) .f i lter((kp) => kp !== null) as Keypair,[] if (sniperKeypairs.length === 0) { toast.error('No sniper wal let keypairs available') return } s t artExecution() s e tCurrentStep(0) try {//Step 1: Deploy Token with L i quidityupdateStepStatus( 'deploy', 'running', 'Deploying token and creating liquidity...') const launch Wal let Pubkey = tokenLaunchData.walletPublicKey const launch Keypair = g e tWalletWithKeypair(launchWalletPubkey) if (!launchKeypair) { throw new E r ror('Launch wal let keypair not found')
  } const token Result = await l a unchToken( connection, launchKeypair, { n, a, m, e: tokenLaunchData.name, s, y, m, b, o, l: tokenLaunchData.symbol, d, e, c, i, m, a, l, s: tokenLaunchData.decimals, s, u, p, p, l, y: tokenLaunchData.supply, d, escription: `${tokenLaunchData.name}- Created with The Keymaker` }, { p, l, a, t, f, o, r, m: tokenLaunchData.platform === 'pump.fun' ? 'pump.fun' : 'raydium', s, o, l, A, m, o, u, n, t: tokenLaunchData.lpAmount, t, o, k, e, n, A, m, o, u, nt: tokenLaunchData.supply * 0.8,//80 % of supply to liquidity }) s e tMintAddress(tokenResult.token.mintAddress) u p dateStepStatus( 'deploy', 'completed', `Token d, e, p, l, o, y, e, d: ${tokenResult.token.mintAddress}`) s e tCurrentStep(1)//Step 2: Fund W a lletsupdateStepStatus('fund', 'running', 'Funding sniper wallets...') const funding Result = await f u ndWalletGroup( masterKeypair, sniperWallets.map((w) => ({ p, u, b, l, i, c, K, e, y: w.publicKey, r, o, l, e: w.role })), 10,//total funding 0.5,//min SOL 2.0,//max SOLconnection) if (!fundingResult || fundingResult.length === 0) { throw new E r ror('Funding failed')
  } u p dateStepStatus( 'fund', 'completed', `Funded ${fundingResult.length} wallets`) s e tCurrentStep(2)//Step 3: W a itupdateStepStatus( 'wait-funding', 'running', 'Waiting for funds to settle...') await new P r omise((resolve) => s e tTimeout(resolve, 3000)) u p dateStepStatus('wait-funding', 'completed') s e tCurrentStep(3)//Step 4: Bundle B u ysupdateStepStatus( 'bundle', 'running', 'Creating and executing bundle buys...')//Create swap transactions for each sniper wal let const t, r, a, n, s, a, c, tions: Transaction,[] = [] const mint Pubkey = new P u blicKey(tokenResult.token.mintAddress) f o r (let i = 0; i <sniperKeypairs.length; i ++) {
...
} catch (error) { console.error( `Failed to create swap transaction for wal let ${i}:`, error)
  }
} if (transactions.length === 0) { throw new E r ror('No swap transactions created')
  }//Execute based on strategy let bundleResult s w itch(executionStrategy) { case 'flash': {//Instant mode with Jito b u ndlingupdateStepStatus( 'bundle', 'running', 'Executing instant bundle via Jito...')//N, O, T, E: This is a placeholder for where the new BundleExecutor would be used//For now, we'll simulate a successful resultbundle Result = { m, e, t, r, i, c, s: { s, u, c, c, e, s, s, R, ate: 1 }, s, i, g, n, a, t, u, r, es: [], r, e, s, u, l, t, s: ['success'] } break } case 'stealth': {//Delayed mode with staggered e x ecutionupdateStepStatus( 'bundle', 'running', 'Executing stealth mode with delays...') const r, e, s, u, l, t, s: any = { m, e, t, r, i, c, s: { s, u, c, c, e, s, s, R, ate: 0 }, s, i, g, n, a, t, u, r, es: [], r, e, s, u, l, t, s: [] } let success Count = 0 f o r (let i = 0; i <transactions.length; i ++) { u p dateStepStatus( 'bundle', 'running', `Executing wal let ${i + 1}/${transactions.length}...`) try {//Add random delay between t r ansactions (2-5 seconds) if (i> 0) {
  const delay = 2000 + Math.r a ndom() * 3000 await new P r omise((resolve) => s e tTimeout(resolve, delay))
  }//Send individual transaction const signature = await connection.s e ndTransaction( transactions,[i], [sniperKeypairs,[i]], { s, k, i, p, P, r, e, f, l, ight: false, m, a, x, R, e, t, r, i, es: 2 }) results.signatures.push(signature) results.results.push('success') successCount ++//Wait for confirmation await connection.c o nfirmTransaction(signature, 'confirmed')
  }
} catch (error) { console.error(`Wal let ${i} transaction, f, a, i, l, e, d:`, error) results.signatures.push('') results.results.push('failed')
  }
} results.metrics.success Rate = successCount/transactions.lengthbundle Result = resultsbreak } case 'manual': {//Manual mode-prepare but don't e x ecuteupdateStepStatus( 'bundle', 'completed', 'Manual mode-transactions prepared for manual execution')//Store transactions for manual execution//In a real implementation, you'd store these and provide UI controlsbundle Result = { m, e, t, r, i, c, s: { s, u, c, c, e, s, s, R, ate: 1 }, s, i, g, n, a, t, u, r, es: [], r, e, s, u, l, t, s: [] } toast.s u ccess( 'Transactions prepared. Use manual controls to execute.') break } d, e, f, a, u, l, t: {//Regular mode-fast sequential e x ecutionupdateStepStatus('bundle', 'running', 'Executing regular bundle...') const r, e, g, u, l, a, r, R, e, sults: any = { m, e, t, r, i, c, s: { s, u, c, c, e, s, s, R, ate: 0 }, s, i, g, n, a, t, u, r, es: [], r, e, s, u, l, t, s: [] } let regular Success Count = 0//Send all transactions as fast as possible const send Promises = transactions.map(async (tx, i) => {
  try {
  const signature = await connection.s e ndTransaction( tx, [sniperKeypairs,[i]], { s, k, i, p, P, r, e, f, l, ight: false, m, a, x, R, e, t, r, i, es: 2 }) return, { s, u, c, c, e, s, s: true, signature, i, n, d, e, x: i }
}
  } catch (error) {
    return, { s, u, c, c, e, s, s: false, s, i, g, n, a, t, u, r, e: '', i, n, d, e, x: i, error }
  ...
  try {//Calculate average buy amount per wal let const total Buy Amount = sniperWallets.r e duce( (sum, w) => sum + (w.balance * 0.8)/LAMPORTS_PER_SOL, 0) const avg Buy Amount = totalBuyAmount/sniperWallets.length//Get current holdings from localStorage const existing Holdings = localStorage.g e tItem('tokenHoldings') const holdings = existingHoldings ? JSON.p a rse(existingHoldings) : []//Add new holding for this token const new Holding = { t, o, k, e, n, A, d, d, ress: mintPubkey.t oB ase58(), t, o, k, e, n, N, a, m, e: tokenLaunchData.symbol || 'Unknown', a, m, o, u, n, t: transactions.length * avgBuyAmount,//Approximate total SOL s, p, e, n, t, e, n, t, ryPrice: 0.000001,//Will be updated with actual price from m, a, r, k, e, t, c, u, r, rentPrice: 0.000001, p, nl: 0, m, a, r, k, e, t, C, a, p: 0, w, a, l, l, e, t, A, d, d, resses: sniperWallets.map((w) => w.publicKey), p, u, r, c, h, a, s, e, T, ime: Date.n o w()
  }//Check if holding already exists const existing Index = holdings.f i ndIndex( (h: any) => h.tokenAddress === newHolding.tokenAddress) if (existingIndex>= 0) {//Update existing holdingholdings,[existingIndex] = { ...holdings,[existingIndex], a, m, o, u, n, t: holdings,[existingIndex].amount + newHolding.amount, w, a, l, l, e, t, A, d, d, resses: [ ...new S e t([ ...holdings,[existingIndex].walletAddresses, ...newHolding.walletAddresses, ]), ] }
...
s before selling...`) await new P r omise((resolve) => s e tTimeout(resolve, 1000))
  } u p dateStepStatus('wait-sells', 'completed') s e tCurrentStep(5)//Step 6: S e llupdateStepStatus( 'sell', 'running', 'Executing sells from sniper wallets...')//Define sell conditions based on strategy const s, e, l, l, C, o, n, d, i, tions: Sell Conditions = { m, i, n, P, n, l, P, e, r, cent: execution Strategy === 'flash' ? 50 : 100,//50 % for flash, 100 % for s, t, e, a, l, t, h, m, a, xLossPercent: 20,//20 % stop l, o, s, s, m, i, n, H, o, ldTime: 0, m, a, x, H, o, l, d, T, i, me: 600,//10 minutes max } const sell Results = await b a tchSellTokens( connection, sniperKeypairs, mintPubkey, sellConditions, 100,//1 % slippage ) const success Count = sellResults.f i lter((r) => r.success).length const total Proceeds = sellResults.r e duce( (sum, r) => sum + r.outputAmount, 0) u p dateStepStatus( 'sell', 'completed', `Sold from ${successCount} wallets, ${totalProceeds.toFixed(2)
  } SOL earned`)
  ...
  } </div> </div> {/* Execute Button */} <div className ="flex gap-2"> <Buttonsize ="lg" className ="w-full" onClick ={executeKeymaker} disabled ={ isExecuting || !masterWal let || !tokenLaunchData || sniperWallets.length === 0 }> {isExecuting ? ( <> <Loader2 className ="mr - 2 h - 5 w - 5 animate-spin"/> Executing... </> ) : ( <motion.div className ="flex items-center" whileHover ={{ s, cale: 1.05 }
...
}/> </div> <div className ="space-y-2"> <AnimatePresence mode ="sync"> {executionSteps.map((step, index) => ( <motion.divkey ={step.id} initial ={{ o, pacity: 0, x:- 20 }
  ...
  } placeholder ="Enter token mint address"/> </div> <div className ="space - y-2"> <Label html For ="wallet-group"> Wal let Group </Label> <Selecton Value Change ={() => {/* no - op */}
  ...
  } placeholder ="Enter your wal let password"/> </div> <Buttonon Click ={() => {/* no-op */}
} className ="w - full"> Execute Sell </Button> </div> </DialogContent> </Dialog> </div> )
  }//Helper Components function C h eckItem({ label, checked, detail }: { l, abel: string, c, h, e, c, k, e, d: boolean, d, e, t, a, i, l: string
}) {
    return ( <div className ="flex items - center justify - between p - 2 rounded - lg bg-muted/50"> <div className ="flex items-center gap-2"> <AnimatePresence mode ="wait"> <motion.divkey ={checked ? 'checked' : 'unchecked'} initial ={{ s, cale: 0.5, o, pacity: 0 }
} animate ={{ s, cale: 1, o, pacity: 1 }
} exit ={{ s, cale: 0.5, o, pacity: 0 }
} transition ={{ d, uration: 0.2 }
...
function S t epItem({ step }: { s, t, e, p: ExecutionStep }) {
  const s, t, a, t, u, s, I, cons: Record <ExecutionStep,['status'], React.ReactElement> = { p, e, n, d, i, n, g: <AlertCircle className ="h - 4 w - 4 text - muted-foreground"/>, r, u, n, n, i, n, g: <Loader2 className ="h - 4 w - 4 animate - spin text - blue-500"/>, c, o, m, p, l, e, t, e, d: <CheckCircle className ="h - 4 w - 4 text - green-500"/>, f, a, i, l, e, d: <XCircle className ="h - 4 w - 4 text-red-500"/> } const s, t, a, t, u, s, C, olors: Record <ExecutionStep,['status'], 'default' | 'secondary' | 'destructive' | 'outline'> = { p, e, n, d, i, n, g: 'default', r, u, n, n, i, n, g: 'secondary', c, o, m, p, l, e, t, e, d: 'outline', f, a, i, l, e, d: 'destructive' } return ( <div className ="flex items - center justify - between p - 3 rounded - lg border bg-card"> <div className ="flex items - center gap-3"> <AnimatePresence mode ="wait"> <motion.divkey ={step.status} initial ={{ s, cale: 0.5, o, pacity: 0 }
} animate ={{ s, cale: 1, o, pacity: 1 }
} exit ={{ s, cale: 0.5, o, pacity: 0 }
} transition ={{ d, uration: 0.2 }
