...
export type FeeEstimate = {
  ...
  totalCost: number;
  costInSol: number;
  perTransaction: { fee: number; tip: number; total: number };
};
...
export function FeeEstimator({
  ...
  const calculateFees = async () => {
    ...
    try {
      ...
      // Use a placeholder key for fee payer and transfer to avoid heavy deps
      const placeholder = new PublicKey('11111111111111111111111111111111');
      sampleTx.add(
        SystemProgram.transfer({
          fromPubkey: placeholder,
          toPubkey: placeholder,
          lamports: 1,
        }),
      );
