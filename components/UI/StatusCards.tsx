...
export function StatusCards() {
  ...
  const [statuses, setStatuses] = useState<StatusData[]>([
    ...
    { name: network === 'mainnet-beta' ? 'Mainnet' : 'Devnet', status: 'offline', latency: 0, history: [] },
  ]);
  ...
  const checkWS = async () => {
    try {
      ...
      return new Promise<{ status: 'online' | 'slow' | 'offline'; latency: number }>((resolve) => {
    ...
    setStatuses(prev => [
      {
        ...prev[0],
        ...rpc,
        history: [...prev[0].history.slice(-29), { time: now, latency: rpc.latency }]
      },
      {
        ...prev[1],
        ...ws,
        history: [...prev[1].history.slice(-29), { time: now, latency: ws.latency }]
      },
      {
        ...prev[2],
        ...jito,
        history: [...prev[2].history.slice(-29), { time: now, latency: jito.latency }]
      },
      {
        ...prev[3],
        name: network === 'mainnet-beta' ? 'Mainnet' : 'Devnet',
        ...net,
        history: [...prev[3].history.slice(-29), { time: now, latency: net.latency }]
      }
    ]);
  };
    ...
    <>
            ...
            {statuses.map((status, idx) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-medium text-white/80">{status.name}</span>
                  <Tooltip>
                    <TooltipTrigger>
                      <div className={`w-2 h-2 rounded-full ${getColorClass(status.status)}`} />
                    </TooltipTrigger>
                    <TooltipContent>
                      <p className="text-xs">
                        {status.status === 'online' ? `${status.latency}ms` : status.status}
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </div>
      ...
      <Dialog open={!!selectedStatus} onOpenChange={() => setSelectedStatus(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{selectedStatus?.name} Status History (30 min)</DialogTitle>
          </DialogHeader>
