export default function BadgePill({ children, tone='muted' }:{ children:React.ReactNode; tone?:'muted'|'accent'|'success'|'warn'|'danger'}) {
  const m = {
  ...
  return <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium ${m}`}>{children}</span>;
}
