...
export function StatusLEDs() {
  const [rpcStatus, setRpcStatus] = useState<HealthStatus>({
    connected: false,
    rtt: 0,
    lastCheck: 0,
  })
  const [wsStatus, setWsStatus] = useState<HealthStatus>({
    connected: false,
    rtt: 0,
    lastCheck: 0,
  })
  const [jitoStatus, setJitoStatus] = useState<HealthStatus>({
    connected: false,
    rtt: 0,
    lastCheck: 0,
  })
    ...
    <>
      <div className="flex items-center gap-2">
        <LED
          status={rpcStatus}
          label="RPC"
          onClick={() => handleLEDClick('rpc')}
        />
        <LED
          status={wsStatus}
          label="WS"
          onClick={() => handleLEDClick('ws')}
        />
        <LED
          status={jitoStatus}
          label="Jito"
          onClick={() => handleLEDClick('jito')}
        />
      </div>
