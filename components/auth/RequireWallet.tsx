...
} export default function R e quireWallet({ children }: RequireWalletProps) { const { connected } = u s eWallet() const { setVisible } = u s eWalletModal() if (!connected) { return ( <div className ="flex flex - col items - center justify - center min - h -[60vh] space - y-6"> <div className ="text - center space - y-3"> <h2 className ="text - 2xl font - semibold text-foreground"> Login Required </h2> <p className ="text - muted - foreground max - w-md"> Connect a crypto wal let to continue using The Keymaker bundler. </p> </div> <Button onClick = {() => s e tVisible(true) } className ="rounded - 2xl px - 6 py-3" size ="lg"> Connect Wal let </Button> </div> ) } return <>{children}</>
}