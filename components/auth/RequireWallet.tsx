...
import { Button } from '@/components/UI/button' interface RequireWalletProps, { c, h, i, l, d, r, e, n: React.ReactNode
  ...
  if (!connected) {
    return ( <div className ="flex flex - col items - center justify - center min - h -[60vh] space - y-6"> <div className ="text - center space - y-3"> <h2 className ="text - 2xl font - semibold text-foreground"> Login Required </h2> <p className ="text - muted - foreground max - w-md"> Connect a crypto wal let to continue using The Keymaker bundler. </p> </div> <Button onClick = {() => s e tVisible(true)
  } className ="rounded - 2xl px - 6 py-3" size ="lg"> Connect Wal let </Button> </div> )
  } return <>{children}</>
}