...
export default function TestBundleForm({
  defaultMode = 'JITO_BUNDLE' as 'JITO_BUNDLE' | 'RPC_FANOUT',
}) {
    ...
    const body =
      mode === 'JITO_BUNDLE'
        ? {
            ...
            perWalletAmount: amount,
            slippageBps: Number(slip),
            jitoTipLamports: Number(fee),
            dryRun: dry,
          }
        : {
            groupId: '',
            ...
            slippageBps: Number(slip),
            priorityFeeMicrolamports: Number(fee),
            concurrency: 3,
            timeoutMs: 12000,
            dryRun: dry,
            cluster: process.env.NEXT_PUBLIC_APP_NETWORK || 'mainnet-beta',
          };
    ...
    <KCard>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <div>
          ...
          <select
            value={mode}
            onChange={(e) => setMode(e.target.value as 'JITO_BUNDLE' | 'RPC_FANOUT')}
            className="w-full bg-zinc-900 border border-k rounded px-2 py-1 text-sm"
          >
        ...
        <div>
          <label className="text-xs text-muted">
            {mode === 'JITO_BUNDLE' ? 'Jito Tip (lamports)' : 'Priority Fee (µlamports)'}
          </label>
          <input
            value={fee}
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <label htmlFor="dry" className="text-xs text-muted">
            Dry-run
          </label>
        </div>
        <div className="md:col-span-2">
          <button
            onClick={run}
            className="w-full bg-zinc-800 hover:bg-zinc-700 rounded px-3 py-2 text-sm"
          >
            Run Test
          </button>
        </div>
