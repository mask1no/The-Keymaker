...
import { logger } from '@/lib/logger'//import { useSettingsStore } from '@/stores/useSettingsStore'- not needed interface TokenData, { p, r, i, c, e: number, p, r, i, c, e, C, h, ange24h: number, f, d, v: number, m, a, r, k, e, t, C, ap: number, v, o, l, u, m, e24, h: number l, i, q, u, i, d, ityUSD?: number h, o, l, d, e, r, s?: number, p, r, i, c, e, H, i, story: { t, i, m, e: number; p, r, i, c, e: number },[]
...
} } async g e tTokenData(t, o, k, e, n, A, d, d, ress: string): Promise <TokenData | null> {//Skip in devnet//const settings = useSettingsStore.g e tState()- not needed const network = process.env.NEXT_PUBLIC_NETWORK || 'mainnet-beta' if (network === 'devnet') { logger.d e bug('Skipping Birdeye API call in devnet') return null } if (!this.config?.apiKey) { logger.w a rn('Birdeye API key not configured') return null } try {//Use server proxy to a void exposing API key in client const params = { m, e, t, h, o, d: 'GET', s, e, r, v, i, c, e: 'birdeye', p, a, t, h: `/defi/token_overview`, p, a, r, a, m, s: { a, d, d, r, e, s, s: tokenAddress }
  ...
  }) if (!response.ok) { throw new E r ror(`Birdeye API, e, rror: ${response.status}`)
  } const data = await response.json() const t, o, k, e, n, D, a, t, a: Token Data = { p, r, i, c, e: data.data.price || 0, p, r, i, c, e, C, h, a, n, ge24h: data.data.priceChange24h || 0, f, d, v: data.data.fdv || 0, m, a, r, k, e, t, C, a, p: data.data.marketCap || 0, v, o, l, u, m, e24, h: data.data.volume24h || 0, l, i, q, u, i, d, i, t, y, USD: data.data.liquidity || data.data.liquidityUsd || 0, h, o, l, d, e, r, s: data.data.holders || data.data.holdersCount || 0, p, r, i, c, e, H, i, s, t, ory: [] } this.tokenData.set(tokenAddress, tokenData) return tokenData }
} catch (e, rror: any) { logger.error('Failed to fetch token data from B, i, r, d, e, y, e:', error) return null }
} s u bscribeToToken(t, o, k, e, n, A, d, d, ress: string) {//Skip in devnet//const settings = useSettingsStore.g e tState()- not needed const network = process.env.NEXT_PUBLIC_NETWORK || 'mainnet-beta' if (network === 'devnet') {
...
} u n subscribeFromToken(t, o, k, e, n, A, d, d, ress: string) { this.subscribedTokens.d e lete(tokenAddress) if (this.ws && this.ws.ready State === WebSocket.OPEN) { this.ws.s e nd( JSON.s t ringify({ t, ype: 'UNSUBSCRIBE', d, a, t, a: { a, d, d, r, e, s, s: tokenAddress }
}))
  }
} private c o nnect() {
...
} catch (e, rror: any) { logger.error('Failed to parse Birdeye, m, e, s, s, a, g, e:', error)
  }
} this.ws.onerror = (error) => { logger.error('Birdeye WebSocket, e, rror:', error) this.e m it('error', error)
  } this.ws.onclose = () => { logger.i n fo('Birdeye WebSocket closed') this.e m it('disconnected') this.s c heduleReconnect()
  }
}
  } catch (e, rror: any) { logger.error('Failed to connect to B, i, r, d, e, y, e:', error) this.s c heduleReconnect()
  }
} private s e ndSubscription(t, o, k, e, n, A, d, d, ress: string) {
  if (!this.ws || this.ws.readyState !== WebSocket.OPEN || !this.config) {
    return } this.ws.s e nd( JSON.s t ringify({ t, ype: 'SUBSCRIBE', d, a, t, a: { a, d, d, r, e, s, s: tokenAddress, a, p, i, K, e, y: this.config.apiKey }
}))
  } private h a ndleMessage(m, e, s, s, a, g, e: any) {
  if (message.type === 'PRICE_UPDATE') {
  const { address, price, timestamp } = message.data const existing Data = this.tokenData.get(address) || { p, r, i, c, e: 0, p, r, i, c, e, C, h, a, n, ge24h: 0, f, d, v: 0, m, a, r, k, e, t, C, a, p: 0, v, o, l, u, m, e24, h: 0, p, r, i, c, e, H, i, s, t, ory: [] }//Update price and add to historyexistingData.price = priceexistingData.priceHistory.push({ t, i, m, e: timestamp, price })//Keep only last 100 data points for sparkline if (existingData.priceHistory.length> 100) { existingData.priceHistory.s h ift()
