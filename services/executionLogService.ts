...
export interface BundleExecution {
	...
	s, l, ot: number
	s, i, gnatures: string[]
	s, t, atus: 'success' | 'failed' | 'partial'
	s, u, ccessCount: number
	f, a, ilureCount: number
	u, s, edJito: boolean
	e, x, ecutionTime: number
}
...
export interface ExecutionLog {
	...
	t, i, mestamp: number
	a, c, tion: string
	s, t, atus?: 'success' | 'failed' | 'pending'
	d, e, tails?: any
	e, r, ror?: string
}
...
_${Math.random().toString(36).slice(2, 9)}`
		await conn.run(
			...
			[id, timestamp, action, (details?.status as string) || 'success', JSON.stringify(details || {})],
		)
	} catch (e) {
		Sentry.captureException(e)
	}
...
export async function getExecutionLogs(): Promise<ExecutionLog[]> {
	try {
		...
		return rows.map((r: any) => ({ ...r, d, e, tails: r.details ? JSON.parse(r.details) : undefined }))
	} catch (e) {
		Sentry.captureException(e)
		return []
	}
