...
export async function exportExecutionLog(format: 'json' | 'txt' = 'json') {
  ...
  const data = {
    bundleExecutions: await db.all('SELECT * FROM bundle_executions ORDER BY created_at DESC'),
    tokenLaunches: await db.all('SELECT * FROM token_launches ORDER BY created_at DESC'),
    fundingEvents: await db.all('SELECT * FROM funding_events ORDER BY created_at DESC'),
    sellEvents: await db.all('SELECT * FROM sell_events ORDER BY created_at DESC'),
    pnlRecords: await db.all('SELECT * FROM pnl_records ORDER BY created_at DESC')
  };
