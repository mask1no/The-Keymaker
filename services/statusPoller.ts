...
export type BundleInflightStatus = { bundle_id: string; status: 'pending'|'landed'|'failed'|'invalid'|'unknown'; landed_slot?: number | null; transactions?: string[] }
...
async function jrpc<T>(region: string, method: string, params: any[], timeout = 8000): Promise<T> {
  ...
  const j = await res.json(); if (j?.error) throw new Error(j.error?.message || `Jito ${method} error`)
  return j.result
}
...
export async function getBundleStatuses(region: string, ids: string[]): Promise<BundleInflightStatus[]> {
  const cache = cacheFor(region); const now = Date.now(); const stale = now - cache.lastFetchMs > 1500
  if (ids.length) {
    try {
      const r: any = await jrpc(region, 'getBundleStatuses', [ids])
      for (const v of (r?.value ?? [])) {
        cache.entries.set(v.bundle_id, { bundle_id: v.bundle_id, status: String(v.status || 'unknown').toLowerCase() as any, landed_slot: v.landed_slot ?? null, transactions: Array.isArray(v.transactions) ? v.transactions : undefined })
      }
    } catch {}
  }
  if (stale && ids.length) {
    try {
      ...
      for (const v of (r2?.value ?? [])) {
        ...
        cache.entries.set(v.bundle_id, { bundle_id: v.bundle_id, status: String(v.status || prev?.status || 'unknown').toLowerCase() as any, landed_slot: v.landed_slot ?? prev?.landed_slot ?? null, transactions: prev?.transactions })
      }
