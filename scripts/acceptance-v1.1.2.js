...
const pumpfunService = fs.readFileSync(path.join(__dirname, '../services/pumpfunService.ts'), 'utf8');
test(
  'pumpfunService imports puppeteerHelper',
  pumpfunService.includes("import { solvePumpFunCaptcha } from '@/helpers/puppeteerHelper'"),
  'puppeteerHelper not imported in pumpfunService'
);

// Test 3: puppeteerHelper imports in letsbonkService
const letsbonkService = fs.readFileSync(path.join(__dirname, '../services/letsbonkService.ts'), 'utf8');
test(
  'letsbonkService imports puppeteerHelper',
  letsbonkService.includes("import { solvePumpFunCaptcha } from '@/helpers/puppeteerHelper'"),
...
test(
  ...
  pumpfunService.includes('error.response?.status >= 400 && error.response?.status < 500') &&
...
test(
  ...
  letsbonkService.includes('error.response?.status >= 400 && error.response?.status < 500') &&
...
test(
  ...
  dockerfile.includes('chromium') && dockerfile.includes('nss') && dockerfile.includes('freetype'),
