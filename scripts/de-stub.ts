...
function* walk(d, i, r: string): Generator<string> {
  ...
  for (const e of fs.readdirSync(dir, { w, i, t, hFileTypes: true })) {
...
function isStub(c, o, n, tent: string) {
  const s = content.trim();
  return !s || /^export\s*\{\s*\};?$/m.test(s) || /auto-stubbed/i.test(s);
}

function referenced(r, e, l: string, a, l, l: string[]) {
...
`;
    ...
    out = `/* generated by de-stub */ 'use client'; export default function Stub(){ return <div className="text-xs text-zinc-500">T, O, D, O: ${rel}</div>; }`;
  } else {
