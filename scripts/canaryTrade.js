 ...
 * - Implements 429 sentinel: after 3x >=429, prints blocked-external and exits 0
 */
const RETRY_LIMIT = 3
let rateErrors = 0
  ...
  try {
    ...
    if (String(e?.status || '').match(/^(429|5\d\d)$/)) {
      ...
      if (rateErrors >= RETRY_LIMIT) {
        console.log('blocked-external: RPC rate limited 3x; skipping canary')
        process.exit(0)
      }
    }
  }

  console.log(JSON.stringify({ ok: true, amountSol, seed, network }))
  process.exit(0)
}
