...
async function checkDatabase(): Promise<HealthCheck> {
  try {
    ...
    const requiredTables = [
      'wallets', 'tokens', 'trades', 'errors', 
      'settings', 'execution_logs', 'pnl_records', 'bundles'
    ];
...
export async function verifyApp() {
  ...
  // Run all checks
  const rpcEndpoint = process.env.NEXT_PUBLIC_HELIUS_RPC || 'https://api.mainnet-beta.solana.com';
  ...
  const result = {
    ...
    checks: checks.reduce((acc, check) => {
      acc[check.name.toLowerCase().replace(/\s+/g, '_')] = check.status === 'ok';
      return acc;
