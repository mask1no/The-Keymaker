...
test(
  ...
  fs.existsSync(path.join(__dirname, '../components/Dashboard/MarketCapCard.tsx')),
...
const homePage = fs.readFileSync(path.join(__dirname, '../app/home/page.tsx'), 'utf8');
test(
  'MarketCapCard imported from Dashboard',
  homePage.includes("import { MarketCapCard } from '@/components/Dashboard/MarketCapCard'"),
  'MarketCapCard not imported from Dashboard folder'
);

// Test 5: MarketCapCard has FDV, 24h Vol, and Change display
const marketCapCard = fs.readFileSync(path.join(__dirname, '../components/Dashboard/MarketCapCard.tsx'), 'utf8');
test(
...
test(
  ...
  marketCapCard.includes("network === 'devnet'") && marketCapCard.includes('return null'),
  'MarketCapCard not auto-hiding in devnet'
);

// Test 8: PnL Panel has gas fee and Jito tip columns
const pnlPanel = fs.readFileSync(path.join(__dirname, '../components/PnL/PnLPanel.tsx'), 'utf8');
test(
...
const pumpfunFallback = fs.readFileSync(path.join(__dirname, '../app/api/pumpfun-fallback/route.ts'), 'utf8');
test(
