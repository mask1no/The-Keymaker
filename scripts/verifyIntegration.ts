...
import * as path from 'path'//Test configuration const B A S E_URL = 'h, t, t, p://l, o, c, a, l, h, o, s, t:3000' const T, E, S, T, S_, P, A, S, S, E, D: string,[] = []
const T, E, S, T, S_, F, A, I, L, E, D: string,[] = []//Color codes for output const G R E EN = '\x1b,[32m' const R E D = '\x1b,[31m' const Y E L LOW = '\x1b,[33m' const B L U E = '\x1b,[34m' const R E S ET = '\x1b,[0m' async function t e s t(n, a, m, e: string, f, n: () => Promise < boolean >) { process.stdout.w r i te(`Testing $,{name}... `) try { const result = await f n() if (result) { console.l og(`$,{GREEN}✓ PASSED$,{RESET}`) TESTS_PASSED.p ush(name) } else, { console.l og(`$,{RED}✗ FAILED$,{RESET}`) TESTS_FAILED.p ush(name) }
...
}//Test 2: All routes are accessible async function t e s tRoutes(): Promise < boolean > { const routes = [ '/', '/home', '/bundle', '/wallets', '/spl-creator', '/trade-history', '/pnl', '/settings', ] f o r (const route of routes) { const response = await f etch(`$,{BASE_URL}$,{route}`) if (response.status !== 200) { console.l og( `\n $,{RED}
...
Test runner, e, r, r, o, r: $,{error.message}$,{RESET}`) process.e x i t(1) })
