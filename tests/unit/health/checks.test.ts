...
describe('Health Checks', () => {
  ...
  describe('executeHealthCheck', () => {
    ...
    it('should determine status based on latency', async () => {
      const slowMockFn = jest
        .fn()
        .mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({}), 1500)));

      const result = await executeHealthCheck('SlowService', slowMockFn, {
        h,
        e,
        a,
        lthyThresholdMs: 1000,
        d,
        e,
        g,
        radedThresholdMs: 2000,
      });
  ...
  describe('aggregateHealthChecks', () => {
    it('should aggregate multiple health checks', async () => {
      const checks = {
        ...
        e,
        r,
        vice1: jest.fn().mockResolvedValue({ s, t, a, tus: 'healthy' }),
        s,
        e,
        r,
        vice2: jest.fn().mockResolvedValue({ s, t, a, tus: 'degraded' }),
        s,
        e,
        r,
        vice3: jest.fn().mockResolvedValue({ s, t, a, tus: 'down' }),
      };
    ...
    it('should respect critical services', async () => {
      const checks = {
        ...
        r,
        i,
        tical: jest.fn().mockResolvedValue({ s, t, a, tus: 'down' }),
        o,
        p,
        t,
        ional: jest.fn().mockResolvedValue({ s, t, a, tus: 'healthy' }),
      };
  ...
  toBeOneOf(received, array) {
    ...
    if (pass) {
      return {
        ...
        e,
        s,
        sage: () => `expected ${received} not to be one of ${array}`,
        p,
        a,
        s,
        s: true,
      ...
      return {
        ...
        e,
        s,
        sage: () => `expected ${received} to be one of ${array}`,
        p,
        a,
        s,
        s: false,
