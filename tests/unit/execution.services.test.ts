...
describe('Execution Services', () => {
  ...
  test('SubmissionService.submitAndPoll handles immediate landed', async () => {
    // Mock jito funcs via global fetch path inside lib or spy directly
    const svc = new SubmissionService('ffm')
    const spySend = jest.spyOn(require('@/lib/server/jitoService'), 'sendBundle').mockResolvedValue({ bundle_id: 'b1' })
    const spyStat = jest
  ...
  test('validateTipAccount returns false for malformed tx', () => {
    // Create a dummy VersionedTransaction-like object to trigger false
