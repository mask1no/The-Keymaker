...
jest.mock('../../lib/crypto', () => ({ encrypt: jest.fn((t: string) => `encrypted_${t}`), decrypt: jest.fn((t: string) => t.replace('encrypted_', '')) }))
jest.mock('bip39', () => ({ generateMnemonic: jest.fn(() => 'mock mnemonic'), mnemonicToSeed: jest.fn(() => Promise.resolve(Buffer.from('mock-seed'))) }))
jest.mock('@solana/web3.js', () => ({
  Keypair: { fromSecretKey: jest.fn(() => ({ publicKey: { toBase58: () => 'mockPublicKey' }, secretKey: Buffer.from('mock-seed'.slice(0, 32)) })) },
}))
