...
export async function healthWithDegradation(): Promise<{
  ...
  for (const service of criticalServices) {
    try {
      const result = await healthCheckWithTimeout(() => checkService(service), 3000, {
        status: 'timeout',
        service,
      });
  ...
  for (const service of optionalServices) {
    try {
      const result = await healthCheckWithTimeout(() => checkService(service), 2000, {
        status: 'degraded',
        service,
        note: 'timeout',
      });
  ...
  switch (service) {
    ...
    case 'redis': {
      ...
      try {
        ...
        return {
          status: ms < 500 ? 'healthy' : 'degraded',
          details: { url, latency_ms: ms },
        } as any;
      } catch (e: any) {
    ...
    case 'external': {
      try {
        const j = await fetch('https://quote-api.jup.ag/v6/health', {
          method: 'HEAD',
          cache: 'no-store',
        });
        const ok = j.ok;
