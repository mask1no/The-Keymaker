...
export function validateTokenParams(params: {
  ...
  if (!params.name || params.name.trim().length === 0) {
    ...
    if (name.length < TOKEN_CONSTRAINTS.NAME_MIN_LENGTH) {
      errors.push(`Token name must be at least ${TOKEN_CONSTRAINTS.NAME_MIN_LENGTH} character`);
    }
    if (name.length > TOKEN_CONSTRAINTS.NAME_MAX_LENGTH) {
      errors.push(`Token name must not exceed ${TOKEN_CONSTRAINTS.NAME_MAX_LENGTH} characters`);
    }
  ...
  if (!params.symbol || params.symbol.trim().length === 0) {
    ...
    const symbol = sanitizeString(params.symbol, TOKEN_CONSTRAINTS.SYMBOL_MAX_LENGTH);
    if (symbol.length < TOKEN_CONSTRAINTS.SYMBOL_MIN_LENGTH) {
      errors.push(`Token symbol must be at least ${TOKEN_CONSTRAINTS.SYMBOL_MIN_LENGTH} character`);
    }
    if (symbol.length > TOKEN_CONSTRAINTS.SYMBOL_MAX_LENGTH) {
      errors.push(`Token symbol must not exceed ${TOKEN_CONSTRAINTS.SYMBOL_MAX_LENGTH} characters`);
    }
  ...
  } else if (params.decimals < TOKEN_CONSTRAINTS.DECIMALS_MIN || params.decimals > TOKEN_CONSTRAINTS.DECIMALS_MAX) {
    errors.push(`Decimals must be between ${TOKEN_CONSTRAINTS.DECIMALS_MIN} and ${TOKEN_CONSTRAINTS.DECIMALS_MAX}`);
  }
  ...
  if (typeof params.supply !== 'number' || isNaN(params.supply)) {
    ...
    errors.push(`Supply must not exceed ${TOKEN_CONSTRAINTS.SUPPLY_MAX.toExponential()}`);
  } else {
  ...
  if (params.description) {
    const desc = sanitizeString(params.description, TOKEN_CONSTRAINTS.DESCRIPTION_MAX_LENGTH);
    if (desc.length > TOKEN_CONSTRAINTS.DESCRIPTION_MAX_LENGTH) {
      errors.push(`Description must not exceed ${TOKEN_CONSTRAINTS.DESCRIPTION_MAX_LENGTH} characters`);
    }
  }
    ...
    if (value) {
      try {
        ...
        if (value.length > TOKEN_CONSTRAINTS.URL_MAX_LENGTH) {
          errors.push(`${field} must not exceed ${TOKEN_CONSTRAINTS.URL_MAX_LENGTH} characters`);
        }
      } catch {
...
export function validateSolAmount(
  ...
  if (amount < TRANSACTION_CONSTRAINTS.MIN_SOL_AMOUNT) {
    errors.push(`Amount must be at least ${TRANSACTION_CONSTRAINTS.MIN_SOL_AMOUNT} SOL`);
  }
  
  if (amount > TRANSACTION_CONSTRAINTS.MAX_SOL_AMOUNT) {
    errors.push(`Amount must not exceed ${TRANSACTION_CONSTRAINTS.MAX_SOL_AMOUNT} SOL`);
  }
  
  // Purpose-specific validation
  if (purpose === 'tip') {
    if (amount < TRANSACTION_CONSTRAINTS.MIN_TIP_AMOUNT) {
      errors.push(`Tip amount must be at least ${TRANSACTION_CONSTRAINTS.MIN_TIP_AMOUNT} SOL`);
    }
    if (amount > TRANSACTION_CONSTRAINTS.MAX_TIP_AMOUNT) {
      errors.push(`Tip amount must not exceed ${TRANSACTION_CONSTRAINTS.MAX_TIP_AMOUNT} SOL`);
    }
  }
 ...
 */
export function validateGroupName(name: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  const sanitized = sanitizeString(name, WALLET_CONSTRAINTS.GROUP_NAME_MAX_LENGTH);
  
  if (sanitized.length > WALLET_CONSTRAINTS.GROUP_NAME_MAX_LENGTH) {
    errors.push(`Group name must not exceed ${WALLET_CONSTRAINTS.GROUP_NAME_MAX_LENGTH} characters`);
  }
 ...
 */
export function validateBundleSize(size: number): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  
  if (!Number.isInteger(size) || size < 1) {
    errors.push('Bundle size must be a positive integer');
  } else if (size > TRANSACTION_CONSTRAINTS.MAX_BUNDLE_SIZE) {
    errors.push(`Bundle size must not exceed ${TRANSACTION_CONSTRAINTS.MAX_BUNDLE_SIZE} transactions`);
  }
 ...
 */
export function validateSlippage(slippageBps: number): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (!Number.isInteger(slippageBps) || slippageBps < 0) {
    ...
    errors.push(`Slippage must not exceed ${TRANSACTION_CONSTRAINTS.MAX_SLIPPAGE_BPS} basis points (50%)`);
  }
 ...
 */
export function validatePublicKey(key: string): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
