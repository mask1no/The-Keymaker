// Input validation utilities for The Keymaker
// - Prevents injection attacks, size overflows, and invalid data
...
export const TOKEN_CONSTRAINTS = {
  ...
  DECIMALS_MAX: 9,
  SUPPLY_MIN: 1,
  // Account for max decimals; just a generous cap for UI-level validation
  SUPPLY_MAX: Number.MAX_SAFE_INTEGER / Math.pow(10, 9),
...
export function validateTokenParams(params: {
  ...
  // Supply validation
  if (typeof params.supply !== 'number' || Number.isNaN(params.supply)) {
...
export function validatePublicKey(key: string): { valid: boolean; errors: string[] } {
  ...
  // Base58 format (rough) 32..44 chars excluding 0 O I l
