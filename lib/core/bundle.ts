...
export async function submitBundle(region: RegionKey, encodedTransactions: string[]): Promise<{ bundle_id: string }>
{
  const result = await jrpc<string>(region, 'sendBundle', { encodedTransactions, bundleOnly: true })
  return { bundle_id: result }
}

export async function fetchBundleStatuses(region: RegionKey, bundleIds: string[]): Promise<BundleStatus[]> {
  return await jrpc<BundleStatus[]>(region, 'getBundleStatuses', bundleIds)
}

export function validateTipAccount(transaction: VersionedTransaction, validRecipients: string[]): boolean {
  try {
