...
function getJitoBase(region?: string) {
  ...
  if (['ffm', 'frankfurt'].includes(r))
    return 'https://frankfurt.mainnet.block-engine.jito.wtf'
  if (['ams', 'amsterdam'].includes(r))
    return 'https://amsterdam.mainnet.block-engine.jito.wtf'
  if (['ny', 'nyc', 'newyork'].includes(r))
    return 'https://ny.mainnet.block-engine.jito.wtf'
  if (['tokyo', 'tyo'].includes(r))
    return 'https://tokyo.mainnet.block-engine.jito.wtf'
  return 'https://frankfurt.mainnet.block-engine.jito.wtf'
}
export function getJitoApiUrl(region?: string) {
  return `${getJitoBase(region)}/api/v1`
}
...
export function validateTipAccount(vt: VersionedTransaction): boolean {
  const keys: string[] = (vt.message.staticAccountKeys || []).map(
