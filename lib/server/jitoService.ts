...
async function withCircuitBreaker<T>(region: RegionKey, method: string, fn: () => Promise<T>): Promise<T> {
  ...
  if (s.state === 'open') {
    if (s.openedAt && now - s.openedAt < COOLDOWN_MS) {
      Sentry.addBreadcrumb({ category: 'circuit', level: 'warning', message: 'Circuit open - short-circuit', data: { region, method, openedAt: s.openedAt } })
      throw new Error(`Circuit open for ${method}`)
    }
    s.state = 'half'
    Sentry.addBreadcrumb({ category: 'circuit', level: 'info', message: 'Circuit half-open', data: { region, method } })
  }

  try {
    const result = await fn()
    if (s.state !== 'closed') {
      Sentry.captureMessage('Circuit closed', { level: 'info', extra: { region, method } })
    }
    s.failures = 0
    s.openedAt = null
    s.state = 'closed'
    ...
    if (shouldOpen) {
      Sentry.captureMessage('Circuit opened', { level: 'warning', extra: { region, method, failures: s.failures } })
      s.state = 'open'
      s.openedAt = now
    }
    throw e
...
async function jrpc<T>(region: RegionKey, method: string, params: unknown, timeoutMs = 10000): Promise<T> {
...
export async function getTipFloor(region: RegionKey = 'ffm'): Promise<TipFloorResponse> {
  ...
  const out = await withCircuitBreaker(region, 'tipfloor', async () =>
    withRetry(
      async () => {
        let res = await fetch(primaryUrl.toString(), { method: 'GET', headers: { 'Content-Type': 'application/json' } })
        if (!res.ok) {
          res = await fetch(rootUrl.toString(), { method: 'GET', headers: { 'Content-Type': 'application/json' } })
        }
...
export async function sendBundle(region: RegionKey, encodedTransactions: string[]): Promise<{ bundle_id: string }> {
  const tryRegions: RegionKey[] = [region, 'ffm', 'ams', 'ny', 'tokyo']
  ...
  for (const r of (enableFailover ? unique : [region])) {
    attempts += 1
    try {
      Sentry.addBreadcrumb({ category: 'jito', level: 'info', message: 'sendBundle attempt', data: { attempt: attempts, region: r, requestedRegion: region } })
      const result = await jrpc<string>(r, 'sendBundle', { encodedTransactions, bundleOnly: true })
      if (r !== region) {
        Sentry.captureMessage('sendBundle region failover succeeded', { level: 'info', extra: { finalRegion: r, attempts } })
      }
      return { bundle_id: result }
    } catch (e) {
      lastError = e
      Sentry.addBreadcrumb({ category: 'jito', level: 'warning', message: 'sendBundle error', data: { attempt: attempts, region: r, error: (e as Error)?.message } })
      continue
    }
  }
  Sentry.captureException(lastError instanceof Error ? lastError : new Error('sendBundle failed'), { extra: { attempts, requestedRegion: region } })
