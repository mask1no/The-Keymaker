...
export async function sendBundle(
  ...
  const res = await fetch(api, {
    ...
    body: JSON.stringify({
      ...
      id: 1,
      method: 'sendBundle',
      params: { encodedTransactions, bundleOnly: true },
    }),
  })
...
export function validateTipAccount(transaction: VersionedTransaction): boolean {
  try {
    ...
    // Ensure last instruction is SystemProgram transfer
    const programIndex = lastIx.programIdIndex
    const programId = msg.staticAccountKeys[programIndex]
    if (!programId || !programId.equals(SystemProgram.programId)) return false

    // Ensure it transfers to a known Jito tip account
    ...
    if (!JITO_TIP_ACCOUNTS.includes(recipient)) return false

    // Enforce minimal lamports if instruction data present
    const data = Buffer.from(lastIx.data)
    if (data.length >= 12) {
      const lamports = data.readBigUInt64LE(4)
      const minLamports = BigInt(1000)
      if (lamports < minLamports) return false
    }
