  ...
  if (s.state === 'open') {
    if (s.openedAt && now - s.openedAt < COOLDOWN_MS) {
      Sentry.addBreadcrumb({
        category: 'circuit',
        level: 'warning',
        message: 'Circuit open - short-circuit',
        data: { region, method, openedAt: s.openedAt },
      });
  ...
  try {
    ...
    if (shouldOpen) {
      Sentry.captureMessage('Circuit opened', {
        level: 'warning',
        extra: { region, method, failures: s.failures },
      });
      s.state = 'open';
      s.openedAt = now;
    }
  ...
  for (const r of enableFailover ? unique : [region]) {
    ...
    try {
      Sentry.addBreadcrumb({
        category: 'jito',
        level: 'info',
        message: 'sendBundle attempt',
        data: { attempt: attempts, region: r, requestedRegion: region },
      });
      ...
      Sentry.addBreadcrumb({
        category: 'jito',
        level: 'warning',
        message: 'sendBundle error',
        data: { attempt: attempts, region: r, error: (e as Error)?.message },
      });
      continue;
    }
  }
  Sentry.captureException(lastError instanceof Error ? lastError : new Error('sendBundle failed'), {
    extra: { attempts, requestedRegion: region },
  });
