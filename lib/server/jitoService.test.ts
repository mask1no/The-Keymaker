...
describe('jitoService', () => {
	describe('getTipFloor', () => {
		it('fetches tip floor successfully', async () => {
			...
			;(globalAny.fetch as jest.Mock).mockResolvedValueOnce({ ok: true, json: () => Promise.resolve(mockResponse) })
			const result = await getTipFloor('ffm' as any)
			expect(globalAny.fetch).toHaveBeenCalledWith(`${JITO_REGIONS.ffm.endpoint}/tipfloor`, expect.any(Object))
	...
	describe('sendBundle', () => {
		it('sends bundle successfully', async () => {
			...
			const result = await sendBundle('ffm' as any, ['encoded-tx-1', 'encoded-tx-2'])
	...
	describe('getBundleStatuses', () => {
		it('gets bundle statuses successfully', async () => {
			const mockStatuses = [
				{ bundle_id: 'bundle-1', transactions: [{ signature: 'sig-1', confirmation_status: 'confirmed' }], confirmation_status: 'landed', slot: 12345 },
			]
			;(globalAny.fetch as jest.Mock).mockResolvedValueOnce({
				ok: true,
				json: () => Promise.resolve({ jsonrpc: '2.0', id: 1, result: mockStatuses }),
			})
			const result = await getBundleStatuses('ffm' as any, ['bundle-1'])
	...
	describe('validateTipAccount', () => {
		it('validates transaction with valid tip account', () => {
			...
			const message = new TransactionMessage({
				...
				instructions: [SystemProgram.transfer({ fromPubkey: payer, toPubkey: tipAccount, lamports: 1000 })],
			}).compileToV0Message()
		...
		it('rejects transaction with invalid tip account', () => {
			...
			const message = new TransactionMessage({
				payerKey: payer,
				recentBlockhash: 'mock-blockhash',
				instructions: [SystemProgram.transfer({ fromPubkey: payer, toPubkey: invalidAccount, lamports: 1000 })],
			}).compileToV0Message()
			const tx = new VersionedTransaction(message)
			const result = validateTipAccount(tx)
