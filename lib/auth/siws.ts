...
export function verifySIWS(params: {
  ...
  // Parse the canonical message format
  const messageParts = message.split('|');
  if (messageParts.length !== 4 || messageParts[0] !== 'Keymaker-Login') {
    return { valid: false, error: 'Invalid message format.' };
  }

  const parsedPubkey = messageParts[1].replace('pubkey=', '');
  const parsedTs = messageParts[2].replace('ts=', '');
  const parsedNonce = messageParts[3].replace('nonce=', '');

  // Verify the nonce matches
  if (parsedNonce !== stored.nonce) {
    return { valid: false, error: 'Nonce mismatch in message.' };
  }

  // Verify the public key matches
  if (parsedPubkey !== pubkey) {
    return { valid: false, error: 'Public key mismatch in message.' };
  }

  // Verify timestamp freshness
  const ts = Date.parse(parsedTs);
...
/**
 * Build SIWS message format - Canonical pipe-delimited format
 */
export function buildSIWSMessage(params: { pubkey: string; nonce: string; ts: string }): string {
  const { pubkey, nonce, ts } = params;
