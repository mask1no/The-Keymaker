...
export const useSettingsStore = create<SettingsState>((set, get) => ({
  ...
  fetchSettings: async () => {
    try {
      ...
      const data = (await response.json()) as Partial<Pick<SettingsState, 'jitoTipLamports' | 'jupiterFeeBps'>>
      set((state) => ({
        ...state,
        jitoTipLamports: typeof data.jitoTipLamports === 'number' ? data.jitoTipLamports : state.jitoTipLamports,
        jupiterFeeBps: typeof data.jupiterFeeBps === 'number' ? data.jupiterFeeBps : state.jupiterFeeBps,
      }))
    } catch (error) {
