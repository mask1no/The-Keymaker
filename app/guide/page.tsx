...
export default function G u idePage() {
  const [content, setContent] = u s eState('')
  const [loading, setLoading] = u s eState(true) u s eEffect(() => { f e tch('/docs/guide.md') .t h en((res) => res.t e xt()) .t h en((text) => { s e tContent(text) s e tLoading(false)
  }) .catch (() => { s e tContent('# Guide\n\nFailed to load guide content.') s e tLoading(false)
  })
  }, [])
  if (loading) {
