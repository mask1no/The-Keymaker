...
export default function HomePage() {
  ...
  const [recentEvents, setRecentEvents] = useState<
    Array<{ id: number; executed_at: string; status: string; outcomes: any }>
  >([]);
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Fetch recent events
        try {
          const recentResponse = await fetch('/api/metrics/recent');
          if (recentResponse.ok) {
            const recentResult = await recentResponse.json();
            setRecentEvents(recentResult.recent || []);
          }
        } catch {
          // API error - use empty array
          setRecentEvents([]);
        }

        // Set wallet count (fetched from API or default to 0)
        setWalletCount(0);
      } catch (error) {
              ...
              {recentEvents.slice(0, 5).map((event) => (
                ...
                >
                  <div className="flex-1">
                    <div className="text-sm text-zinc-200">Bundle #{event.id}</div>
