    ...
    if (simulateOnly) {
      ...
      try {
        for (const tx of transactions) {
          const result = await connection.simulateTransaction(tx, { sigVerify: false, commitment: 'processed' as any })
          if (result.value.err) {
            return NextResponse.json({ error: `Transaction simulation failed: ${JSON.stringify(result.value.err)}` }, { status: 400 })
          }
        }
