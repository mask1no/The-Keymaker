...
function partition(total: number, n: number, strategy: 'equal'|'random'): number[] {
  if (n <= 0) return [];
  if (strategy === 'equal') return Array.from({length:n}, (_,i)=> Math.floor(total/n) + (i < (total % n) ? 1 : 0));
  const cuts = Array.from({length:n-1}, ()=> Math.random()).sort((a,b)=>a-b);
  const parts: number[] = []; let prev = 0;
...
export async function POST(req: Request) {
  ...
  const conn = new Connection(process.env.HELIUS_RPC_URL || 'https://api.mainnet-beta.solana.com', { commitment: 'processed' });

  const blockhash = await conn.getLatestBlockhash('finalized');
  const payer = new PublicKey(g.masterWallet);

  const unsigned = dests.map((to, i) => {
    const ix = SystemProgram.transfer({ fromPubkey: payer, toPubkey: new PublicKey(to), lamports: amounts[i] });
    const msg = new TransactionMessage({ payerKey: payer, recentBlockhash: blockhash.blockhash, instructions: [ix] }).compileToV0Message();
