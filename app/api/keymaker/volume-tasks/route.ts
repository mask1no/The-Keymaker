...
export async function GET(request: NextRequest) {
  ...
  try {
    const db = getDb();
    const tasks = db.all('SELECT * FROM volume_tasks WHERE user_id = ? ORDER BY created_at DESC', [
      session.sub,
    ]);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const existing = db.get('SELECT id FROM volume_tasks WHERE user_id = ? AND name = ?', [
      session.sub,
      validatedData.name,
    ]);
