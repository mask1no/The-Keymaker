...
const RATE_LIMIT = { windowMs: 60 * 1000, maxRequests: 100 }
...
function checkRateLimit(ip: string): boolean {
  ...
  if (limit.count >= RATE_LIMIT.maxRequests) return false
  limit.count++
  return true
}

/** Validate request parameters */
function validateRequest(service: string, p: string, params: any): { valid: boolean; error?: string } {
  ...
  const isPathAllowed = serviceConfig.allowedPaths.some((allowedPath) => p.startsWith(allowedPath))
  if (!isPathAllowed) {
    return { valid: false, error: 'Path not allowed' }
  }
  // Validate specific parameters based on service
  if (service === 'birdeye' && p.includes('/token/')) {
    const tokenAddress = p.split('/token/')[1]
  ...
  if (params) {
    Object.keys(params).forEach((key) => {
      if (typeof (params as any)[key] === 'string') {
        (params as any)[key] = sanitizeString((params as any)[key], 200)
      }
    })
  }
  return { valid: true }
...
async function executePythonMCP(method: string, params: any): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    const pythonProcess = spawn(
      'python',
      [path.join(mcpPath, 'src', 'bonk_mcp', 'cli_wrapper.py'), '--method', method, '--params', JSON.stringify(params)],
      {
        cwd: mcpPath,
        env: {
          ...process.env,
...
export async function POST(request: Request) {
  try {
    // Require signed session for any proxy and especially MCP calls
    ...
    const { service, path: reqPath, params, method = 'GET' } = body

    // Handle Python MCP calls behind auth
    ...
    // Validate request
    const validation = validateRequest(service, reqPath, params)
    if (!validation.valid) {
    ...
    // Build request URL
    const url = new URL(reqPath, serviceConfig.baseUrl)
    ...
    if (!response.ok) {
      console.error(`API proxy error: ${service}${reqPath}`, { status: response.status, error: data, ip })
    }

    // Return proxied response
    return NextResponse.json(data, {
      status: response.status,
      headers: { 'X-RateLimit-Remaining': String(RATE_LIMIT.maxRequests - (rateLimitMap.get(ip)?.count || 0)) },
    })
  } catch (error) {
