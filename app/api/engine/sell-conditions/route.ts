...
async function fetchCurrentPriceLamportsPerToken(mint: string): Promise<number | null> {
  try {
    ...
    // We don't have SOL/USD here; this placeholder returns null so percent targets rely on client-provided entryPrice
    // Future: incorporate SOL price to convert USD → lamports
    const priceUsd = Number(j?.priceUsd || 0);
    if (!priceUsd || !Number.isFinite(priceUsd)) return null;
    return null;
  } catch { return null; }
}
...
export async function POST(request: Request) {
  try {
    ...
    let scheduled: Array<{ id: string; afterMs: number; percent: number }> = [];
    ...
    let executed: Array<{ id: string; percent: number }> = [];
