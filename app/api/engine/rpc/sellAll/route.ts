...
import { multiWalletSellAll } from '@/lib/engine/trade';
...
export const POST = withSessionAndLimit(async (request) => {
  try {
    ...
    const validated = sellAllSchema.parse(body);
    ...
    // Execute multi-wallet sell all
    const results = await multiWalletSellAll({
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Failed to execute sell all',
      },
      { status: 500 },
    );
  }
