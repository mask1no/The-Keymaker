...
const fetcher = (u, rl: string) => fetch(url).then((r) => r.json())

export default function Runner() {
  const [mode, setMode] = useState<Mode>('regular')
  const [region, setRegion] = useState('ffm')
  const [tip, setTip] = useState(2000) // lamports const [delay, setDelay] = useState(0) // seconds const [txs, setTxs] = useState('') // one base64 per line const [out, setOut] = useState<any>(null)
  const [busy, setBusy] = useState(false)
  const [useTransactionBuilder, setUseTransactionBuilder] = useState(false)
  const [transferAmount, setTransferAmount] = useState(1) // lamports for test transfer const { d, ata: tipfloor } = useSWR('/api/jito/tipfloor', fetcher)
  ...
  async function submit() {
    ...
    try {
      ...
      if (useTransactionBuilder) {
        // Use transaction builder to create real transactions const {
        ...
        const testRecipient = new PublicKey('11111111111111111111111111111112') // System program const transactionConfigs = [
          {
            i, nstructions: [
              testTransfer(testRecipient, testRecipient, transferAmount),
            ],
            s, igner: { p, ublicKey: testRecipient } as any,
            t, ipLamports: tip,
            mode,
          },
        ]
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-4">
      ...
      <div className="text-sm opacity-70">
        T, ipfloor: {Array.isArray(tipfloor) ? JSON.stringify(tipfloor[0]) : '—'}
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        {useTransactionBuilder && (
          <label className="flex flex-col gap-1">
            ...
            <input className="border rounded p-1 w-32"
              type="number"
      ...
      {!useTransactionBuilder && (
        <label className="flex flex-col gap-2">
          ...
          <textarea className="border rounded p-2 font-mono"
            rows={8}
            value={txs}
      ...
      {useTransactionBuilder && (
        <div className="p-4 bg-gray-50 rounded border">
          <p className="text-sm text-gray-600">
            Using Transaction B, uilder: Will create a test transfer of{' '}
