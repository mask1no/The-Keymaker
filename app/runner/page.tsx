...
export default function Runner() {
    ...
    try {
      ...
      if (useTransactionBuilder) {
        ...
        const transactionConfigs = [{
          instructions: [createTestTransferInstruction(testRecipient, testRecipient, transferAmount)],
        ...
        const serialized = serializeBundleTransactions(bundleTxs)
        txs_b64 = serialized.transactions
      } else {
      ...
      const res = await fetch('/api/bundles/submit', {
        ...
        body: JSON.stringify({ region, txs_b64, tip_lamports: tip, mode, delay_seconds: delay }),
      })
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <label className="flex flex-col gap-2">
          ...
          <select className="border rounded p-2" value={region} onChange={e=>setRegion(e.target.value)}>
            <option>ffm</option><option>ldn</option><option>nyc</option>
            <option>slc</option><option>sgp</option><option>tyo</option><option>ams</option>
          </select>
        </label>
        <label className="flex flex-col gap-2">
          <span>Jito tip (lamports)</span>
          <input className="border rounded p-2" type="number" value={tip} onChange={e=>setTip(parseInt(e.target.value||'0'))}/>
        </label>
        <label className="flex flex-col gap-2">
          <span>Delay (seconds, for delayed)</span>
          <input className="border rounded p-2" type="number" value={delay} onChange={e=>setDelay(parseInt(e.target.value||'0'))}/>
        </label>
      </div>
      ...
      {!useTransactionBuilder && (
        <label className="flex flex-col gap-2">
          <span>Base64 Versioned Transactions (one per line, last tx must include a Jito tip account in static keys)</span>
          <textarea className="border rounded p-2 font-mono" rows={8} value={txs} onChange={e=>setTxs(e.target.value)} placeholder="AAAA..."/>
        </label>
      )}
      ...
      {useTransactionBuilder && (
        <div className="p-4 bg-gray-50 rounded border">
          <p className="text-sm text-gray-600">
            Using Transaction Builder: Will create a test transfer of {transferAmount} lamports with embedded Jito tip.
          </p>
        </div>
      )}

      <button disabled={busy} onClick={submit} className="px-4 py-2 rounded bg-emerald-600 text-white">
