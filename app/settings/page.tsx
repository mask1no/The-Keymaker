    ...
    const loadSettings = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setSettings({
            rpcUrl: data.customFees.rpc.buyPriorityLamports > 0 ? 'custom' : 'default',
            jitoEnabled: data.customFees.jito.enabled,
            jitoRegion: 'ffm', // Default region
            jitoPriority: data.customFees.rpc.preset || 'med',
            tipLamports: data.customFees.jito.buyTipLamports,
            slippageTolerance: data.customFees.slippageBpsDefault / 100,
            maxGasPrice: 0.01, // Default value
            chunkSize: 5, // Default value
            concurrency: 4, // Default value
            hotkeys: {
              devSell: 'Ctrl+D',
              sellAll: 'Ctrl+S',
              emergencyStop: 'Ctrl+E',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await apiFetch('/api/ui/settings', {
        ...
        body: JSON.stringify({
          customFees: {
            useCustomFees: settings.rpcUrl === 'custom',
            rpc: {
              buyPriorityLamports: settings.rpcUrl === 'custom' ? 5000 : 0,
              sellPriorityLamports: settings.rpcUrl === 'custom' ? 5000 : 0,
              cuLimit: 900000,
              preset: settings.jitoPriority as 'low' | 'med' | 'high' | 'vhigh',
              autoPriority: true,
            },
            jito: {
              enabled: settings.jitoEnabled,
              buyTipLamports: settings.tipLamports,
              sellTipLamports: settings.tipLamports,
            },
            slippageBpsDefault: Math.round(settings.slippageTolerance * 100),
          },
        }),
      });
