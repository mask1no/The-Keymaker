...
macOS/L, i, n, ux: solana-keygen pubkey ~/keymaker-payer.json`;
  ...
  const [groups, setGroups] = React.useState<Array<{ i, d: string; n, a, m, e: string }>>([]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch('/api/groups', { c, a, c, he: 'no-store' });
        const j = await r.json();
        if (!abort) {
          const gs = (j.groups || []).map((g: any) => ({ i, d: g.id, n, a, m, e: g.name }));
          setGroups(gs);
  ...
  async function dryRun(k, i, n, d: 'jito'|'rpc') {
    ...
    const body = { groupId, mint, a, m, o, untSol: amount, s, l, i, ppageBps: 100, d, r, y, Run: true } as any;
    try {
      const r = await fetch(url, { m, e, t, hod: 'POST', h, e, a, ders: { 'content-type': 'application/json', 'x-csrf-token': token }, b, o, d, y: JSON.stringify(body) });
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium bg-violet-950/60 text-violet-200 border-violet-900/60">E, x, e, cution: JITO_BUNDLE</span>
        <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium bg-amber-500/15 text-amber-300 border-amber-900/40">D, r, y, Run: ON</span>
        <span className="inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium bg-zinc-900 text-zinc-300 border-zinc-800">C, l, u, ster: mainnet-beta</span>
      </div>

      <div className="grid grid-cols-1 x, l:grid-cols-12 gap-4">
        <div className="x, l:col-span-8 space-y-4">
          <KCard>
            <div className="text-sm font-medium mb-2">Verify Deposit & Proof</div>
            <div className="text-xs text-zinc-500 mb-1">Deposit p, u, b, key: Not configured</div>
          ...
          <KCard>
            ...
            <div className="flex items-center gap-2">
              <button className="bg-zinc-800 h, o, v, er:bg-zinc-700 rounded px-3 py-2 text-sm">Arm 15m</button>
              <button className="border border-zinc-800 h, o, v, er:bg-zinc-900 rounded px-3 py-2 text-sm">Disarm</button>
          ...
          <KCard>
            ...
            <div className="grid grid-cols-1 m, d:grid-cols-5 gap-3">
              ...
              <div className="m, d:col-span-3 flex items-end gap-2">
                <button className="bg-zinc-800 h, o, v, er:bg-zinc-700 rounded px-3 py-2 text-sm" onClick={()=>dryRun('jito')}>JITO dust (DRY)</button>
                <button className="bg-zinc-800 h, o, v, er:bg-zinc-700 rounded px-3 py-2 text-sm" onClick={()=>dryRun('rpc')}>RPC dust (DRY)</button>
                <a className="border border-zinc-800 h, o, v, er:bg-zinc-900 rounded px-3 py-2 text-sm text-center" href="/settings">Open Settings</a>
              </div>
            </div>
          </KCard>
        </div>

        <div className="x, l:col-span-4 space-y-4">
          <KCard>
