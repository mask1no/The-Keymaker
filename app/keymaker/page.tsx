...
export default function KeymakerPage() {
  ...
  const { data: feed } = useSWR(() => (group?.mint ? `/api/mint/activity?mint=${group.mint}&limit=50` : null), fetcher);
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <div className="flex items-center gap-2">
        <button className={`chip ${tab==='manual'?'active':''}`} onClick={() => setTab('manual')}>Manual</button>
        <button className={`chip ${tab==='volume'?'active':''}`} onClick={() => setTab('volume')}>Volume</button>
      </div>
      {tab==='manual' ? (
      ...
      <div className="flex items-center gap-3">
        <button className="button" onClick={() => void 0}>Fund from Master</button>
        <button className="button" onClick={() => void 0}>Sweep SOL</button>
        <div className="ml-auto flex gap-2">
          {[25, 50, 75, 100].map((p) => (
            <button key={p} className="chip">SELL {p}%</button>
          ))}
        </div>
      </div>
      {tab==='volume' ? (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
        {profiles.map((p) => (
          <div key={p.id} className="flex items-center gap-2 p-2 rounded border border-zinc-800">
            ...
            <button className="chip ml-auto" onClick={() => start(p.id)}>Run</button>
          </div>
        ))}
      </div>
      <div className="flex items-center gap-2">
        <div className="text-xs text-zinc-400">RunId: {runId || '—'}</div>
        <button className="chip" onClick={stop} disabled={!runId}>Stop</button>
        <button className="chip" onClick={() => mutate()}>Refresh</button>
      </div>
...
function VolumeStatus({ runId }: { runId: string }) {
  ...
  const { data } = useSWR(`/api/volume/status?runId=${encodeURIComponent(runId)}`, fetcher, { refreshInterval: 3000 });
  const run = data?.run;
  return (
